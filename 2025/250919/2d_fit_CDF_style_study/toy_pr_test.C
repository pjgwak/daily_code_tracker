...
void toy_pr_test()
{
  ...
  // extend
  RooRealVar *N_b = new RooRealVar("N_{b}", "signal number B Jpsi K", 8000, 1, 10000);
  ...
  // --- NP ---
  RooRealVar *N_np = new RooRealVar("N_{NP}", "signal number B Jpsi K", 7000, 1, 100000);
  ...
  RooRealVar *slope = new RooRealVar("slope", "slope", 0.01, -0.1, 0.11);
  RooAbsPdf *mass_BB_pdf = new RooPolynomial("mass_QCD_pdf", "mass pdf QCD", *mass, *slope);
  
  // prompt BB time - NP인데 Res과 함쳤다는 뜻?
  RooRealVar *b_tau_bkg = new RooRealVar("c#tau_{bkg}", "B ctau", 0.1, 0.04, 0.2, "cm");
  RooAbsPdf *time_BB_pdf = new RooDecay("time_BB_pdf", "time pdf B2 Jpsi K", *tom, *b_tau_bkg, *properTimeRes, RooDecay::SingleSided);
  RooRealVar *N_BB = new RooRealVar("N_{BB}", "Signal number BB", 5000, 1, 10000);
  ...
  c1.SaveAs("figs/toy_pr_ctau.png");
  // c1.SaveAs("figs/toy_pr_ctau.pdf");
  }
  ...
  c2.SaveAs("figs/toy_pr_mass.png");
  // c2.SaveAs("figs/toy_pr_mass.pdf");
  }

  result->Print("v");

  cout << "=== finish toy_pr_test()";
  t.Stop();
