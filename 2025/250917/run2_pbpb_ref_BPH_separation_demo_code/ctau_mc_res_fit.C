...
void ctau_mc_res_fit()
{
  ...
  // === fit by Expo ===
  RooRealVar mu0("mu0", "res mean", 0.003, -0.02, 0.02);
  // RooRealVar mu0("mu0", "res mean", 0);

  RooRealVar sigma1("sigma1", "sigma1", 0.3, 0.1, 1);
  RooRealVar r21("r21", "sigma2/sigma1", 1.4, 1, 5);
  RooFormulaVar sigma2("sigma2", "@0*@1", RooArgList(sigma1, r21));

  RooRealVar r32("r32", "sigma3/sigma2", 1.1, 1, 5);
  ...
  RooFormulaVar fg1("fg1", "@0*@1", RooArgList(fsum, fg12));
  RooFormulaVar fg2("fg2", "@0*(1-@1)", RooArgList(fsum, fg12));
  // RooRealVar fg2("fg2", "frac g2", 0.5, 0.01, 1);
  ...
  // --- perform fit --- //
  auto fitResult = model.fitTo(*ds_red, Save(), Range("fitRange"), SumW2Error(hasWeight), Offset(true), PrintLevel(-1), NumCPU(32), EvalBackend("legacy"), Warnings(kFALSE), Verbose(kFALSE), RecoverFromUndefinedRegions(1.2), PrintEvalErrors(-1), Minimizer("Minuit2", "MINIMIZE"));
  ...
  latexParams.DrawLatex(x_start, y_start + y_step * y_stepCount++, Form("f_{G1+G2} = %.3f #pm %.3f", fsum.getVal(), fsum.getError()));
  latexParams.DrawLatex(x_start, y_start + y_step * y_stepCount++, Form("f_{G1}/f_{G1+G2} = %.3f #pm %.3f", fg12.getVal(), fg12.getError()));
