...
void drawFinalCtau(RooWorkspace *ws, RooDataSet *redDataCut, RooDataHist* binDataCtErr, float NSigNP_fin, float NBkg_fin, float Bfrac_fin, float ErrBfrac_fin, RooFitResult *fit2D, float lmin, float lmax, double* UnNormChi2_time_t, int* nFitParam_time_t, int* nFullBinsPull_time_t, int* Dof_time_t,double* Chi2_time_t) {
  ...
  *Chi2_time_t = chi2;
  ...
  redDataCut->plotOn(tframefill,DataError(RooAbsData::SumW2),Binning(rb),MarkerSize(1));
    //ws->pdf("totPDF_PEE")->plotOn(tframefill,DrawOption("F"),FillColor(kBlack),FillStyle(3354),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization(1,RooAbsReal::NumEvent));
    ws->pdf("totPDF_PEE")->plotOn(tframefill,DrawOption("F"),FillColor(kGray+2),FillStyle(3354),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization(1,RooAbsReal::NumEvent));
    ws->pdf("totPDF_PEE")->plotOn(tframefill,LineColor(kBlack),LineWidth(2),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization(1,RooAbsReal::NumEvent));
    RooAddPdf tmpPDF2("tmpPDF2","tmpPDF2",RooArgList(*(ws->pdf("MassCtNP")),*(ws->pdf("MassCtBkg"))),RooArgList(tmpVar1,tmpVar2));
    tmpPDF2.plotOn(tframefill,DrawOption("F"),FillColor(kWhite),FillStyle(1001),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization((NSigNP_fin+NBkg_fin)/redDataCut->sumEntries(),RooAbsReal::NumEvent));
    //tmpPDF2.plotOn(tframefill,DrawOption("F"),FillColor(kRed),FillStyle(3444),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization((NSigNP_fin+NBkg_fin)/redDataCut->sumEntries(),RooAbsReal::NumEvent));
    tmpPDF2.plotOn(tframefill,DrawOption("F"),FillColor(kRed-7),FillStyle(3345),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization((NSigNP_fin+NBkg_fin)/redDataCut->sumEntries(),RooAbsReal::NumEvent));
    gStyle->SetHatchesLineWidth(2);
    //ws->pdf("totPDF_PEE")->plotOn(tframefill,Components("MassCtBkg"),DrawOption("F"),FillColor(kAzure-9),FillStyle(1001),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization(1,RooAbsReal::NumEvent));
    ws->pdf("totPDF_PEE")->plotOn(tframefill,Components("MassCtBkg"),DrawOption("F"),FillColor(kBlue-10),FillStyle(1001),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization(1,RooAbsReal::NumEvent));
    //ws->pdf("totPDF_PEE")->plotOn(tframefill,Components("MassCtBkg"),LineColor(kBlue),LineWidth(5),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization(1,RooAbsReal::NumEvent),LineStyle(7));
    ws->pdf("totPDF_PEE")->plotOn(tframefill,Components("MassCtBkg"),LineColor(kBlue-2),LineWidth(5),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization(1,RooAbsReal::NumEvent),LineStyle(7));
    //tmpPDF2.plotOn(tframefill,LineColor(kRed),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization((NSigNP_fin+NBkg_fin)/redDataCut->sumEntries(),RooAbsReal::NumEvent),LineWidth(5),LineStyle(9));
    tmpPDF2.plotOn(tframefill,LineColor(kPink-6),ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*binDataCtErr,kTRUE),NumCPU(32),Normalization((NSigNP_fin+NBkg_fin)/redDataCut->sumEntries(),RooAbsReal::NumEvent),LineWidth(5),LineStyle(11));
...
void fit2D_622()
{
  ...
  // --- PEE(= errPdfBkg)와의 곱 ---
  ws->factory("PROD::CtBkgPos_PEE(errPdfBkg, CtBkgPos)");
  ws->factory("PROD::CtBkgNeg_PEE(errPdfBkg, CtBkgNeg)");
  ws->factory("PROD::CtBkgDbl_PEE(errPdfBkg, CtBkgDbl)");
  ws->factory("PROD::CtPRRes_PEE(errPdfBkg, CtPRRes)");

  // --- 원래 합 구조를 _PEE 버전으로 재구성 ---
  ws->factory("SUM::CtBkgSum1_PEE(fracCtBkg1*CtBkgPos_PEE, CtBkgNeg_PEE)");
  ws->factory("SUM::CtBkgSum2_PEE(fracCtBkg2*CtBkgSum1_PEE, CtBkgDbl_PEE)");
  ws->factory("SUM::CtBkgTot_PEE(fracCtBkg3*CtPRRes_PEE, CtBkgSum2_PEE)");

  // ws->factory("PROD::CtBkgTot_PEE(errPdfBkg|ctau3DErr, CtBkgTot|ctau3D)");
  ...
  RooFitResult *fitCt_Bkg = ws->pdf("CtBkgTot_PEE")->fitTo(*redDataSB, SumW2Error(kTRUE), Minos(0), Save(1), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))), Optimize(0), NumCPU(1), EvalBackend("legacy"), PrintEvalErrors(-1), PrintLevel(-1));
  // EvalBackend("legacy"),
  fitCt_Bkg->Print("v");
...

  {
    ...
    RooRealVar* ctErr = ws->var("ctau3DErr");
    RooAbsPdf*  pdf   = ws->pdf("CtBkgTot_PEE");

    // bin 수: 있으면 그대로, 없으면 120
    int nBins = ctau->getBinning().numBins();
    if (nBins <= 1) nBins = 120;

    // --- frame ---
    RooPlot* f = ctau->frame(Bins(nBins), Title("Ct sideband fit (with PEE)"));
    // 데이터 (SumW2 에러)
    redDataSB->plotOn(f, Binning(nBins), DataError(RooAbsData::SumW2), Name("h_data_ct"));
    // 모델 (PEE는 조건부)

    ctau->setRange("ctauNorm", -0.5, 2);

    // pdf->plotOn(f, Name("model_ct"), LineColor(kBlue), LineWidth(3));
    
    pdf->plotOn(f, Name("model_ct"), LineColor(kBlue), LineWidth(3), Range("ctauNorm"), NormRange("ctauNorm"), ProjWData(RooArgList(*(ws->var("ctau3DErr"))),*redDataSB, kTRUE), Normalization(redDataSB->sumEntries(),RooAbsReal::NumEvent));

    // ProjWData(RooArgList(*(ws->var("ctau3D"))),*redData,kTRUE), Normalization(redDataSB->sumEntries(),RooAbsReal::NumEvent)

    // // --- pull ---
    // RooHist* hp = f->pullHist("h_data_ct", "model_ct");
    // hp->SetName("hpull_ct");
    // RooPlot* fPull = ctau->frame(Title("Pull(ctau3D)"));
    // fPull->addPlotable(hp, "P");
    // fPull->GetYaxis()->SetRangeUser(-5, 5);

    // // --- chi2/ndf ---
    // double chi2 = 0; int nFull = 0;
    // for (int i = 0, n = hp->GetN(); i < n; ++i) {
    //   double x, y; hp->GetPoint(i, x, y);
    //   if (std::isfinite(y)) { chi2 += y*y; ++nFull; }
    // }
    // const int nPar = fitCt_Bkg ? fitCt_Bkg->floatParsFinal().getSize() : 0;
    // const int dof  = std::max(1, nFull - nPar);
    // const double chi2ndf = chi2 / dof;

    // --- draw & save ---
    TCanvas c("c_ctau_sb","ctau sideband",800,700);
    f->Draw();
    // c.Divide(1,2,0,0);
    // c.cd(1); f->GetYaxis()->SetTitle("Events / bin"); f->Draw();
    // { TLegend leg(0.62,0.68,0.88,0.88); leg.SetBorderSize(0); leg.SetFillStyle(0);
    //   leg.AddEntry(f->findObject("h_data_ct"), "Data(SB)", "lep");
    //   leg.AddEntry(f->findObject("model_ct"),  "Model",    "l");
    //   leg.Draw();
    //   TLatex t; t.SetNDC(true); t.SetTextSize(0.035);
    //   t.DrawLatex(0.15,0.84,Form("#chi^{2}/ndf = %.2f / %d = %.3f", chi2, dof, chi2ndf));
    // }
    // c.cd(2); fPull->Draw();

    c.SaveAs("sb_ctau_fit.png");
    c.SaveAs("sb_ctau_fit.pdf");

    // std::cout << "[CtSB] bins="<<nFull<<", nPar="<<nPar
    //           <<", chi2="<<chi2<<", dof="<<dof
  ...
  // {
  //   // --- 준비 ---
  //   RooAbsPdf* pdf = ws->pdf("totPDF_PEE");
  //   RooRealVar* mass = ws->var("mass");
  //   RooRealVar* ctau = ws->var("ctau3D");
  //   RooRealVar* ctErr = ws->var("ctau3DErr");

  //   // bin 수: 워크스페이스에 미리 정의된 binning이 있으면 사용, 없으면 120
  //   int nMassBins = (mass->hasBinning("cache") ? mass->getBinning("cache").numBins()
  //                                             : mass->getBinning().numBins());
  //   if (nMassBins <= 1) nMassBins = 120;

  //   // --- Mass frame ---
  //   RooPlot* fMass = mass->frame(Bins(nMassBins), Title("Final mass projection"));
  //   // 데이터 (SumW2 에러 사용)
  //   redData->plotOn(fMass,
  //                   Binning(nMassBins),
  //                   DataError(RooAbsData::SumW2),
  //                   Name("h_data_mass"));

  //   // 모델 곡선 (ctau, ctErr 위로 데이터-가중 사영 + ctErr는 조건부)
  //   pdf->plotOn(fMass,LineColor(kBlack),LineWidth(2), ProjWData(RooArgList(*(ws->var("ctau3D"))),*redData,kTRUE), NumCPU(16), Precision(-1), Name("model_mass"), Normalization(1,RooAbsReal::NumEvent));
  //   // Normalization(NSigNP_fin+NBkg_fin,RooAbsReal::NumEvent)
    
  //   // pdf->plotOn(fMass,
  //   //         ProjWData(RooArgSet(*ws->var("ctau3D")), *redData),        // 적분할 변수만
  //   //         ConditionalObservables(RooArgSet(*ws->var("ctau3DErr"))),  // 조건부(PEE)
  //   //         Name("model_mass"),
  //   //         LineColor(kBlue), LineWidth(2));
  //   // --- Pull plot ---
  //   RooHist* hpull_mass = fMass->pullHist("h_data_mass", "model_mass");
  //   hpull_mass->SetName("hpull_mass");

  //   RooPlot* fPull = mass->frame(Title("Pull(mass)"));
  //   fPull->addPlotable(hpull_mass, "P");
  //   fPull->GetYaxis()->SetTitle("(data - model) / #sigma");
  //   fPull->GetYaxis()->SetRangeUser(-5, 5);
  ...
  //   {
  //     // RooHist 내부 포인트 접근
  //     const int n = hpull_mass->GetN();
  //     for (int i = 0; i < n; ++i) {
  //       double x, y;
  //       hpull_mass->GetPoint(i, x, y);           // y = pull 값
  //       const double ey = hpull_mass->GetErrorY(i);
  //       if (std::isfinite(y)) {                  // pull은 이미 (resid/err) 이므로 ey는 보통 0
  //         chi2 += y * y;
  //         ++nFull;
  //       }
  //     }
  //   }
  //   const int nPar = fit2D ? fit2D->floatParsFinal().getSize() : 0;
  //   const int dof  = std::max(1, nFull - nPar);
  //   const double chi2ndf = chi2 / dof;

  //   // --- 캔버스 저장 ---
  //   TCanvas cMass("cMass", "Final Mass", 800, 700);
  //   cMass.Divide(1, 2, 0, 0);
  //   cMass.cd(1);
  //   fMass->GetYaxis()->SetTitle("Events / bin");
  //   fMass->Draw();

  //   // 범례
  //   TLegend leg(0.62, 0.68, 0.88, 0.88);
  //   leg.SetBorderSize(0); leg.SetFillStyle(0);
  //   leg.AddEntry(fMass->findObject("h_data_mass"), "Data", "lep");
  //   leg.AddEntry(fMass->findObject("model_mass"), "Model (proj. w/ data)", "l");
  //   leg.Draw();

  //   // χ²/ndf 표기
  //   TLatex lat;
  //   lat.SetNDC(true); lat.SetTextSize(0.035);
  //   lat.DrawLatex(0.15, 0.84, Form("#chi^{2}/ndf = %.2f / %d = %.3f", chi2, dof, chi2ndf));

  //   cMass.cd(2);
  //   fPull->Draw();

  //   cMass.SaveAs("final_mass.png");
  //   cMass.SaveAs("final_mass.pdf");

  //   // 필요하면 수치 로그 출력
  //   std::cout << "[FinalMass] bins used = " << nFull
  //             << ", nPar = " << nPar
  //             << ", chi2 = " << chi2
  //             << ", dof = " << dof
  //             << ", chi2/ndf = " << chi2ndf << std::endl;
