...
void mass_fit()
{
  ...
  // variables
  RooRealVar *massVar = dynamic_cast<RooRealVar *>(dsReduced->get()->find("mass"));
  if (!massVar)
  ...
  // --- Extended yields ---
  RooRealVar Nsig("Nsig", "signal yield", 15000, 1, 100000000);
  RooRealVar Nbkg("Nbkg", "background yield", 1.2305e+02, 1, 10000000);

  RooAddPdf model("pdf_mass_tot", "",
                RooArgList(sig, bkg), RooArgList(Nsig, Nbkg));

  auto dh = new RooDataHist("dh", "binned dataset", *massVar, *dsReduced);
  ...
  auto fitResult = model.fitTo(*dsReduced,
                               RooFit::Save(), SumW2Error(hasWeight), Offset(true),
                               Extended(true),
                               NumCPU(32),
                               EvalBackend("legacy"),
  ...
  dsReduced->plotOn(massFrame, DataError(RooAbsData::SumW2), Name("data"));
  ...
  // model.plotOn(massFrame, Range("SR"), NormRange("SR"), VLines(), FillColor(kAzure - 9), FillStyle(3004), DrawOption("F"), Name("model_sub"));
  ...
  latexInfo.DrawLatex(x_start, y_start + y_step * y_stepCount++, Form("Data"));
  if (yLow == 0)
  ...
  c_mass.SaveAs(Form("figs/mass_pT%.1f_%.1f_y%.1f_%.1f.png", ptLow, ptHigh, yLow, yHigh));
  c_mass.SaveAs(Form("figs/mass_pT%.1f_%.1f_y%.1f_%.1f.pdf", ptLow, ptHigh, yLow, yHigh));

  fitResult->Print("V");
  cout << "\n chi2/ndf = " << chi2ndf << endl;

  // === save results ===
  TFile fout(Form("roots/mass_pT%.1f_%.1f_y%.1f_%.1f.root", ptLow, ptHigh, yLow, yHigh), "RECREATE");
