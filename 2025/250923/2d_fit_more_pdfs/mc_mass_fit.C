...
void mc_mass_fit()
{
  ...
  RooRealVar nRatio("nRatio", "nR/nL ratio", 0.6, 0.1, 5);
  ...
  // --- gauss ---
  RooRealVar sigmaGRatio("sigmaGRatio", "", 1.0, 0.5, 5.0);
  ...
  RooAddPdf model("sig", "", RooArgList(cbLeft, cbRight, gaus), RooArgList(f_cbR, f_gaus), kTRUE);


  // === perform fit ===
  auto fitResult = model.fitTo(*dsReduced, Save(), Range("fitRange"), SumW2Error(hasWeight), Offset(true), PrintLevel(-1), Warnings(kFALSE), Verbose(kFALSE), NumCPU(32), EvalBackend("legacy"));
