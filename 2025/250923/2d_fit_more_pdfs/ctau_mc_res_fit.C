...
void ctau_mc_res_fit()
{
  ...
  // variables
  RooRealVar *ctau3DVar = dynamic_cast<RooRealVar *>(dsReduced->get()->find("ctau3DRes"));
  if (!ctau3DVar)
  ...
  RooRealVar fg1("fg1", "frac g1", 0.1, 0.1, 1);
  RooRealVar fg2("fg2", "frac g2", 0.1, 0.1, 1);
  ...
  auto fitResult = model.fitTo(*dsReduced,
                               Save(), Range("fitRange"), SumW2Error(hasWeight), Offset(true),
                               Extended(true),
                               NumCPU(32),
                               EvalBackend("legacy"),
