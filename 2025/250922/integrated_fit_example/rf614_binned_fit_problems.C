...
// Unfortunately, it has a problem: it also has the bin bais that this tutorial
// demonstrates, to if we would use it, the biases would cancle out.
...
void rf614_binned_fit_problems()
{
  ...
  // If you do the fit the usual was in RooFit, you will get a bias in the result.
  // This is because the continuous normalized pdf is evaluated only at the bin centers.
  std::unique_ptr<RooFitResult> fit1{expo.fitTo(*expoData, Save(), PrintLevel(-1), SumW2Error(false))};
  fit1->Print();

  // 1) 히스토그램과 expo PDF 플롯
  RooPlot *frame = x.frame(Title("Exponential Fit with Binned Asimov Data"));
  expoData->plotOn(frame, MarkerStyle(20), MarkerSize(0.8));
  expo.plotOn(frame);

  // 2) Canvas 생성 및 저장
  TCanvas c1("c1", "c1", 800, 600);
  frame->Draw();
  c1.SaveAs("expo_fit_plot_fit1.png"); // PNG로 저장

  // In the case of an exponential function, the bias that you get by evaluatiing the pdf only at the bin centers is a constant scale factor in
  ...
  // the bias cancles out. You can achieve this by customizing the way how the
  ...
  frame = x.frame(Title("Exponential Fit with Binned Asimov Data"));
  expoData->plotOn(frame, MarkerStyle(20), MarkerSize(0.8));
  expo.plotOn(frame);

  // 2) Canvas 생성 및 저장
  frame->Draw();
  c1.SaveAs("expo_fit_plot_fit2.png"); // PNG로 저장
  ...
  RooPowerSum powerlaw{"powerlaw", "poweralw", x, RooConst(1.0), a};
  ...
  // 그림: powerlaw 데이터 + powerlaw PDF
  RooPlot *frame2 = x.frame(Title("Powerlaw Fit with Binned Asimov Data"));
  powerlawData->plotOn(frame2, MarkerStyle(20), MarkerSize(0.8));
  powerlaw.plotOn(frame2);

  TCanvas c2("c2", "c2", 800, 600);
  frame2->Draw();
  c2.SaveAs("powerlaw_fit_plot_fit3.png");
  ...
  // 그림: powerlaw 데이터 + powerlaw PDF
  frame2 = x.frame(Title("Powerlaw Fit with Binned Asimov Data"));
  powerlawData->plotOn(frame2, MarkerStyle(20), MarkerSize(0.8));
  powerlaw.plotOn(frame2);

  frame2->Draw();
  c2.SaveAs("powerlaw_fit_plot_fit4.png");
  ...
  std::unique_ptr<RooFitResult> fit5{powerlaw.fitTo(*powerlawData, IntegrateBins(1e-3), Save(), PrintLevel(-1), SumW2Error(false))};
  fit5->Print();
  frame2 = x.frame(Title("Powerlaw Fit with Binned Asimov Data"));
  powerlawData->plotOn(frame2, MarkerStyle(20), MarkerSize(0.8));
  powerlaw.plotOn(frame2);
  ...
  frame2->Draw();
  c2.SaveAs("powerlaw_fit_plot_fit5.png");
  ...
  RooPlot *frame3 = x.frame(Title("Gaussian + Exponential Model Fit"));
  modelData->plotOn(frame3, MarkerStyle(20), MarkerSize(0.6));
  model.plotOn(frame3);

  TCanvas c3("c3", "c3", 800, 600);
  frame3->Draw();
  c3.SaveAs("gauss_expo_model_fit_plot_fit6.png");
  ...
  std::unique_ptr<RooFitResult> fit7{model.fitTo(*modelData, Offset("bin"), Save(), PrintLevel(-1), SumW2Error(false))};
  fit7->Print();

  frame3 = x.frame(Title("Gaussian + Exponential Model Fit"));
  modelData->plotOn(frame3, MarkerStyle(20), MarkerSize(0.6));
  model.plotOn(frame3);

  frame3->Draw();
  c3.SaveAs("gauss_expo_model_fit_plot_fit7.png");
}