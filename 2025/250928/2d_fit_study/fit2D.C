...
void drawCtauSBPlots(RooWorkspace *ws, RooDataSet *redDataSB, RooDataHist *binDataCtErrSB, RooFitResult *fitCt_Bkg, float lmin, float lmax, double *UnNormChi2_side_t, int *nFitParam_side_t, int *nFullBinsPull_side_t, int *Dof_side_t, double *Chi2_side_t)
{
  ...
  ws->pdf("CtBkgTot_PEE")->plotOn(tframe1, ProjWData(RooArgList(*(ws->var("ctau3DErr"))), *binDataCtErrSB, kTRUE), NumCPU(8), Normalization(1, RooAbsReal::NumEvent), LineStyle(7));
  // ws->pdf("CtBkgTot_PEE")->plotOn(tframe1, NumCPU(8), LineStyle(7));
...
void fit2D()
{
  ...
  double ctMin = -0.1, ctMax = 0.4; // lmin
  float errmin = 0.008, errmax = 0.2;
  ...
  // error PDF
  RooHistPdf errPdfSig("errPdfSig", "Error PDF signal", RooArgSet(*(ws->var("ctau3DErr"))), *binSubtractedSIG, 2);
  ws->import(errPdfSig);
  ...
  fit2D = ws->pdf("totPDF_PEE")->fitTo(*redData, Minos(0), Save(1), SumW2Error(kTRUE), NumCPU(8), RecoverFromUndefinedRegions(1), PrintEvalErrors(-1), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))));
