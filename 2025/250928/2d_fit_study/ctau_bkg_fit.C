...
void ctau_bkg_fit(string region = "LSB")
{
  ...
  TString osCut = "(recoQQsign == 0) && (ctau3D > -0.2 && ctau3D < 0.5 &&ctau3DErr > 0 && ctau3DErr < 0.3)";
  ...
  ctau3D->setRange(-0.2, 0.5);
  ctau3D->setRange("fitRange", -0.2, 0.5);
  ctau3DErr->setRange(0.00, 0.3);
  ctau3DErr->setRange("errRange", 0.00, 0.3);
  ...
  auto h2 = dynamic_cast<RooHistPdf *>(fin.Get("errPdfBkg")->Clone("errPdfBkgInter2"));
  ...
  RooRealVar tau_mid("tau_mid", "scale factor middle", 0.1, 0.01, 1.0);
  RooRealVar tau_mid2("tau_mid2", "scale factor middle", 0.1, 0.01, 1.0);

  // ratio parameters
  RooRealVar r_left("r_left", "tau_left / tau_mid", 5, 0.001, 3);

  
  RooRealVar r_right("r_right", "tau_right / tau_mid", 5, 0.1, 100);
  ...
  auto decayBkgMid1 = new RooDecay("decayBkgMid1", "Center decay",
                                  *ctau3D, tau_mid, *resModel, RooDecay::DoubleSided);
  auto decayBkgMid2 = new RooDecay("decayBkgMid2", "Center decay",
                                  *ctau3D, tau_mid, *resModel, RooDecay::DoubleSided);

  RooRealVar fmid12("fmid12", "tau_right / tau_mid", 0.3, 0, 1);
  auto decayBkgMid = new RooAddPdf("decayBkgMid", "sum of 2 decays",
                                   RooArgList(*decayBkgMid1, *decayBkgMid2),
                                   RooArgList(fmid12));

  auto decayBkgR = new RooDecay("decayBkgR", "Right decay",
  ...
  // auto fr = CtPR_PEE.fitTo(*dsReduced, Save(), Range("fitRange"), Offset(true), PrintLevel(0), PrintEvalErrors(-1), NumCPU(32), EvalBackend("legacy"));

  auto fr = CtPR_PEE.fitTo(*dsReduced, Save(), Range("fitRange"), Offset(true), PrintLevel(-1), PrintEvalErrors(-1), ConditionalObservables(RooArgSet(*ctau3DErr)), NumCPU(32), EvalBackend("legacy"));
