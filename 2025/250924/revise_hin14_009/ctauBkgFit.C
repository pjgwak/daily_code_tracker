...
void setWSRange(RooWorkspace *ws, float lmin, float lmax, float errmin, float errmax)
{
  ...
  ws->var("ctau3Dtrue")->setRange(0.0001, lmax);
  ...
  ws->var("ctau3D")->setRange("ctuaSBFit", -0.2, 0.4);
...
void defineCtBkg(RooWorkspace *ws)
{
  ...
  ws->factory("SUM::CtBkgSum1(fracCtBkg1[0.3,0.01,1.0]*CtBkgPos,CtBkgNeg)");
  ws->factory("SUM::CtBkgSum2(fracCtBkg2[0.1,0.01,1.0]*CtBkgSum1,CtBkgDbl)");
  ws->factory("SUM::CtBkgTot(fracCtBkg3[0.1,0.01,1.0]*CtPRRes,CtBkgSum2)");
  return;
}
...
void drawCtNPTrue(RooWorkspace *ws, RooDataSet *redNPMC, string titlestr)
{
  ...
  cout << " " << endl;
  cout << " -*-*-*-*-*-*-*-*-*-*-*-*-*- !!!!!FITTING!!!!! *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-  " << endl;
  ws->pdf("CtNPTrue")->fitTo(*redNPMCTrue, Minos(0), SumW2Error(kTRUE), NumCPU(8)); //, EvalBackend("legacy")
...
void defineCtNP(RooWorkspace *ws, RooDataSet *redNPMC, string titlestr)
{
  ...
  // simple ctau NP pdf
  ws->factory("GaussModel::BResTrue(ctau3Dtrue,mean[0.0],sigmaNPTrue[0.00002,0.0000001,0.00003])"); // resolution from B meson (before CtPRRes)
  ws->factory("Decay::CtNPTrue(ctau3Dtrue,coefExpNPTrue[0.4,0.25,0.65],BResTrue,RooDecay::SingleSided)");
...
void drawCtauSBPlots(RooWorkspace *ws, RooDataSet *redDataSB, RooDataHist *binDataCtErrSB, RooFitResult *fitCt_Bkg, float lmin, float lmax, double *UnNormChi2_side_t, int *nFitParam_side_t, int *nFullBinsPull_side_t, int *Dof_side_t, double *Chi2_side_t)
{
  ...
  RooPlot *tframe1 = ws->var("ctau3D")->frame();
  double avgBinWidth = rb.averageBinWidth();
  ...
  // original
  ws->pdf("CtBkgTot_PEE")->plotOn(tframe1, ProjWData(RooArgList(*(ws->var("ctau3DErr"))), *binDataCtErrSB, kTRUE), NumCPU(16), Normalization(1, RooAbsReal::NumEvent), LineStyle(7), NumCPU(32));
...
void drawFinalCtau(RooWorkspace *ws, RooDataSet *redDataCut, RooDataHist *binDataCtErr, float NSigNP_fin, float NBkg_fin, float Bfrac_fin, float ErrBfrac_fin, RooFitResult *ctauBkgFit, float lmin, float lmax, double *UnNormChi2_time_t, int *nFitParam_time_t, int *nFullBinsPull_time_t, int *Dof_time_t, double *Chi2_time_t)
{
  char reduceDS[512];
  string titlestr;
  ...
  *UnNormChi2_time_t = chi2;
  int nFitPar = ctauBkgFit->floatParsFinal().getSize();
  dof = nFullBins - nFitPar;
  chi2 /= (nFullBins - nFitPar);
...
void drawFitCt_PRMC(RooWorkspace *ws, RooDataSet *redPRMC, RooFitResult *fitCt_PRMC)
{
  RooRealVar *ctau3D = ws->var("ctau3D");
  RooPlot *frame = ctau3D->frame(Title("Fit result: CtPR_PEE vs redPRMC"));

  redPRMC->plotOn(frame, DataError(RooAbsData::SumW2));

  ws->pdf("CtPR_PEE")->plotOn(frame, ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redPRMC), LineColor(kBlue), LineWidth(2));

  RooHist *hpull = frame->pullHist();

  double chi2 = frame->chiSquare(fitCt_PRMC->floatParsFinal().getSize());
  std::cout << "[CtPR_PEE] chi2/ndf = " << chi2 << std::endl;

  RooPlot *frame_pull = ctau3D->frame(Title("Pull distribution"));
  ...
  // Linear scale + pull
  // ---------------------------------------------------
  TCanvas *c_lin = new TCanvas("c_fitCt_PRMC_lin", "Fit result (linear)", 800, 800);
  c_lin->Divide(1, 2);
  ...
  frame->Draw();
  {
    ...
    leg1->AddEntry((TObject *)0, "Fit: CtPR_PEE", "");
  ...
  frame_pull->Draw();
  TLine *line0_lin = new TLine(ctau3D->getMin(), 0, ctau3D->getMax(), 0);
  line0_lin->SetLineStyle(2);
  line0_lin->Draw("same");

  c_lin->SaveAs("fitCt_PRMC_lin.pdf");

  // ---------------------------------------------------
  // Log scale + pull
  // ---------------------------------------------------
  TCanvas *c_log = new TCanvas("c_fitCt_PRMC_log", "Fit result (log)", 800, 800);
  c_log->Divide(1, 2);
  ...
  frame->Draw();
  {
    ...
    leg2->AddEntry((TObject *)0, "Fit: CtPR_PEE", "");
  ...
  gPad->SetPad(0.0, 0.0, 1.0, 0.3);
  frame_pull->Draw();
  TLine *line0_log = new TLine(ctau3D->getMin(), 0, ctau3D->getMax(), 0);
  line0_log->SetLineStyle(2);
  line0_log->Draw("same");

  c_log->SaveAs("fitCt_PRMC_log.pdf");
}

void drawFitCt_NPMC(RooWorkspace *ws, RooDataSet *redNPMC, RooFitResult *fitCt_NPMC)
{
  RooRealVar *ctau3D = ws->var("ctau3D");
  RooPlot *frame = ctau3D->frame(Title("Fit result: extBkg vs redNPMC"));

  redNPMC->plotOn(frame, DataError(RooAbsData::SumW2));
  ...
  ws->pdf("CtBkgPos")->plotOn(frame, ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redNPMC), LineColor(kBlue), LineStyle(kDashed), Normalization(redNPMC->sumEntries() * (1 - ws->var("fracCtBkg3")->getVal()) * ws->var("fracCtBkg2")->getVal() * ws->var("fracCtBkg1")->getVal(), RooAbsReal::NumEvent));
  RooCurve *curvePos = (RooCurve *)frame->getObject(frame->numItems() - 1);

  // --- CtBkgNeg ---
  ws->pdf("CtBkgNeg")->plotOn(frame, ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redNPMC), LineColor(kGreen), LineStyle(kDashed), Normalization(redNPMC->sumEntries() * (1 - ws->var("fracCtBkg3")->getVal()) * ws->var("fracCtBkg2")->getVal() * (1 - ws->var("fracCtBkg1")->getVal()), RooAbsReal::NumEvent));
  RooCurve *curveNeg = (RooCurve *)frame->getObject(frame->numItems() - 1);

  // --- CtBkgDbl ---
  ws->pdf("CtBkgDbl")->plotOn(frame, ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redNPMC), LineColor(kOrange), LineStyle(kDashed), Normalization(redNPMC->sumEntries() * (1 - ws->var("fracCtBkg3")->getVal()) * (1 - ws->var("fracCtBkg2")->getVal()), RooAbsReal::NumEvent));
  RooCurve *curveDbl = (RooCurve *)frame->getObject(frame->numItems() - 1);

  // --- CtPRRes (Resolution) ---
  ws->pdf("CtPRRes")->plotOn(frame,
                             ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redNPMC),
                             LineColor(kMagenta), LineStyle(kDashed),
                             Normalization(
                                 redNPMC->sumEntries() *
                                     ws->var("fracCtBkg3")->getVal(),
                                 RooAbsReal::NumEvent));
  RooCurve *curveRes = (RooCurve *)frame->getObject(frame->numItems() - 1);
  ...
  frame->addObject(sumAll);

  // ws->pdf("extBkg")->plotOn(
  //     frame,
  //     ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redNPMC),
  //     LineColor(kBlue),
  //     LineWidth(2),
  //     Range("ctuaSBFit"),
  //     NormRange("ctuaSBFit"),
  //     Normalization(redNPMC->sumEntries(), RooAbsReal::NumEvent));

  // ws->pdf("extBkg")->plotOn(frame, ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redNPMC), LineColor(kBlue), LineWidth(2), Range("ctuaSBFit"), NormRange("ctuaSBFit")); // 83816
  // , Normalization(20000, RooAbsReal::NumEvent)

  RooHist *hpull = frame->pullHist();

  double chi2 = frame->chiSquare(fitCt_NPMC->floatParsFinal().getSize());
  std::cout << "[extBkg] chi2/ndf = " << chi2 << std::endl;
  ...
  // Linear scale + pull
  // ---------------------------------------------------
  TCanvas *c_lin = new TCanvas("c_fitCt_NPMC_lin", "Fit result (linear)", 800, 800);
  c_lin->Divide(1, 2);
  ...
  {
    TLegend *leg1 = new TLegend(0.6, 0.7, 0.88, 0.88);
    leg1->AddEntry((TObject *)0, "Fit: extBkg", "");
    leg1->AddEntry((TObject *)0, Form("#chi^{2}/ndf = %.2f", chi2), "");
    leg1->Draw();
  }
  ...
  c_lin->SaveAs("fitCt_NPMC_lin.pdf");

  // ---------------------------------------------------
  // Log scale + pull
  // ---------------------------------------------------
  TCanvas *c_log = new TCanvas("c_fitCt_NPMC_log", "Fit result (log)", 800, 800);
  c_log->Divide(1, 2);
  ...
  {
    TLegend *leg2 = new TLegend(0.6, 0.7, 0.88, 0.88);
    leg2->AddEntry((TObject *)0, "Fit: CtBkgTot_PEE", "");
    leg2->AddEntry((TObject *)0, Form("#chi^{2}/ndf = %.2f", chi2), "");
    leg2->Draw();
  }
  ...
  c_log->SaveAs("fitCt_NPMC_log.pdf");
}

void ctauBkgFit()
{
  cout << "=== start ctauBkgFit() ===\n";
  TStopwatch t;
  t.Start();
  ...
  double ctMin = -0.2, ctMax = 0.4; // lmin
  float errmin = 0.008, errmax = 1.0;
  ...
  setWSRange(ws, -ctMin, 0.4, errmin, errmax);
  ...
  // ws->import(CtBkgTot_PEE);
  // ws->factory("PROD:CtBkgTot_PEE(CtBkgTot|ctau3DErr, errPdfBkg)");

  // Build 2D PDF (mass x ctau) :: for step[[6]]
  sprintf(funct, "PROD::MassCtPR(%s,CtPRRes)", "G1CB1Sig");
  ws->factory(funct);
  sprintf(funct, "PROD::MassCtNP(%s,CtNPTot)", "G1CB1Sig");
  ws->factory(funct);
  sprintf(funct, "PROD::MassCtBkg(%s,CtBkgTot)", "expBkg");
  ws->factory(funct);
  ...
  // drawFitCt_PRMC(ws, redPRMC, fitCt_PRMC);
  drawCtauResolPlots(ws, true, tempFramePR);

  // [5] ctau bkg fit
  // ----------------
  cout << " *** DATA :: N events to fit on SIDEBANDS : " << redDataSB->sumEntries() << endl;
  cout << " *** DATA :: N events to fit on binDataCtErrSB : " << binDataCtErrSB->sumEntries() << endl;
  ...
  // drawFitCt_NPMC(ws, redDataSB, fitCt_Bkg);

  cout << "=== finish ctauBkgFit() ===\n";
  t.Stop();
