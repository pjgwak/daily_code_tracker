...
void fit2D_622()
{
  ...
  RooFitResult *fitCt_Bkg = ws->pdf("CtBkgTot_PEE")->fitTo(*dhDataSB, SumW2Error(kTRUE), Minos(0), Save(1), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))), Optimize(0), NumCPU(32), PrintEvalErrors(-1), PrintLevel(-1));
  // EvalBackend("legacy"),
  fitCt_Bkg->Print("v");
  ...
  drawCtauSBPlots(ws, dhDataSB, binDataCtErrSB, fitCt_Bkg, -ctMin, ctMax, &UnNormChi2_side, &nFitParam_side, &nFullBinsPull_side, &Dof_side, &Chi2_side);
