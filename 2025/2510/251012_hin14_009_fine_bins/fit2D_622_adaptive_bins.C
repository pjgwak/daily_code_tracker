...
RooBinning buildEqualExpectedBinningWithPeak(RooAbsPdf &pdf, RooRealVar &var,
                                             RooDataSet *data,
                                             double vmin, double vmax,
                                             double targetPeak = 1.0e4,
                                             int minBins = 60, int maxBins = 4000,
                                             int grid = 20000,
                                             double tol = 0.15) // +/-15% band
{
  // -------- Helpers (local lambdas) --------
  auto make_cdf = [&](std::vector<double> &x, std::vector<double> &cdf)
  {
    // Numerical CDF over [vmin, vmax] using simple trapezoids
    x.resize(grid + 1);
    cdf.resize(grid + 1);
    double dx = (vmax - vmin) / grid;
    double norm = 0.0;
    for (int i = 0; i <= grid; i++)
    {
      double xv = vmin + i * dx;
      var.setVal(xv);
      double fv = pdf.getVal(RooArgSet(var)); // unnormalized density along var
      x[i] = xv;
      if (i == 0 || i == grid)
        norm += 0.5 * fv;
      else
        norm += fv;
      cdf[i] = (i == 0 ? 0.0 : cdf[i - 1]) + ((i == 0 ? 0.0 : 0.5 * (fv + pdf.getVal(RooArgSet(var))))) * dx; // filled below again
    }
    // recompute properly (two passes for clarity)
    double acc = 0.0;
    for (int i = 0; i <= grid; i++)
    {
      var.setVal(vmin + i * (vmax - vmin) / grid);
      double fv = pdf.getVal(RooArgSet(var));
      if (i == 0 || i == grid)
        acc += 0.5 * fv;
      else
        acc += fv;
      cdf[i] = (i == 0 ? 0.0 : cdf[i - 1] + 0.5 * (fv + pdf.getVal(RooArgSet(var))) * ((vmax - vmin) / grid)); // will renorm next
    }
    // normalize CDF to 1
    double Z = cdf.back();
    if (Z <= 0)
    {
      // fallback to uniform if pdf returned zeros
      for (int i = 0; i <= grid; i++)
        cdf[i] = double(i) / grid;
    }
    else
  ...
  auto edges_from_exp = [&](double expPerBin, std::vector<double> &edges)
  {
    // Build edges so that each bin holds expPerBin expected events
    // Number of bins will be ~ totalYield/expPerBin within [minBins,maxBins].
    double totalY = (data ? data->sumEntries() : 0.0);
    if (totalY <= 0)
      totalY = 1.0; // avoid div0
    int nbins = std::clamp(int(std::round(totalY / expPerBin)), minBins, maxBins);
    // Prepare CDF
    std::vector<double> xs, cdf;
    make_cdf(xs, cdf);
    edges.clear();
    edges.reserve(nbins + 1);
    edges.push_back(vmin);
    // target CDF levels
    for (int k = 1; k < nbins; ++k)
    {
      double tk = double(k) / nbins; // quantile
      // linear search on grid (binary would also be fine)
      auto it = std::lower_bound(cdf.begin(), cdf.end(), tk);
      int idx = std::clamp(int(it - cdf.begin()), 1, (int)cdf.size() - 1);
      // linear interpolate between grid points
      double t0 = cdf[idx - 1], t1 = cdf[idx], x0 = xs[idx - 1], x1 = xs[idx];
      double w = (t1 > t0) ? (tk - t0) / (t1 - t0) : 0.0;
      edges.push_back(x0 + w * (x1 - x0));
    }
    edges.push_back(vmax);
  };

  auto peak_from_edges = [&](const std::vector<double> &edges)
  {
    // Fill data histogram with proposed edges and return the peak bin content
    if (!data)
      return 0.0;
    TH1D h("__tmp__", "", (int)edges.size() - 1, &edges[0]);
    h.Sumw2();
    for (int i = 0; i < data->numEntries(); ++i)
    {
      const RooArgSet *row = data->get(i);
      double v = static_cast<const RooRealVar *>(row->find(var.GetName()))->getVal();
      h.Fill(v);
    }
    return h.GetMaximum();
  };

  // -------- Initial expPerBin guess from targetPeak --------
  double totalY = (data ? data->sumEntries() : 0.0);
  if (totalY <= 0)
    totalY = targetPeak * 100; // arbitrary fallback
  // Bound expPerBin so that nbins within [minBins, maxBins]
  double expMin = totalY / maxBins, expMax = totalY / minBins;
  double expPerBin = std::clamp(targetPeak, expMin, expMax);

  // -------- Iterative retune to hit peak ~ targetPeak --------
  std::vector<double> edges;
  for (int iter = 0; iter < 6; ++iter)
  { // few iterations are enough
    edges_from_exp(expPerBin, edges);
    double pk = peak_from_edges(edges);
    if (pk <= 0)
      break;
    double rel = (pk - targetPeak) / targetPeak;
    if (std::abs(rel) <= tol)
      break; // within tolerance
    // If peak is too high -> more bins -> smaller expPerBin
    if (rel > 0)
    {
      expPerBin = std::max(expMin, expPerBin / (1.0 + rel * 0.8));
    }
    else
    {
      expPerBin = std::min(expMax, expPerBin / (1.0 + rel * 0.8)); // rel<0 increases expPerBin
    }
  }

  // Build RooBinning
  RooBinning rb(edges.size() - 1, &edges[0]);
  return rb;
}
...
void defineCtBkg(RooWorkspace *ws)
{
  ...
  ws->factory("Decay::CtBkgNeg(ctau3D,lambdam[0.02,0.01, 2],CtPRRes,RooDecay::Flipped)");
...
void drawCtNPTrue(RooWorkspace *ws, RooDataSet *redNPMC, string titlestr)
{
  ...
  cout << " " << endl;
  cout << " -*-*-*-*-*-*-*-*-*-*-*-*-*- !!!!!FITTING!!!!! *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-  " << endl;
  ws->pdf("CtNPTrue")->fitTo(*redNPMCTrue, Minos(0), NumCPU(32), Range("trueRange")); //, EvalBackend("legacy"), SumW2Error(kTRUE)
...
{
  RooRealVar *ctVar = ws->var("ctau3D");
  const int initBins = 100;
  const double xmin = lmin;
  const double xmax = lmax;

  // // 임시 히스토그램 생성
  // TH1D *hTemp = new TH1D("hTemp", "temporary histogram", initBins, xmin, xmax);
  // for (int i = 0; i < redDataSB->numEntries(); ++i)
  // {
  //   const RooArgSet *row = redDataSB->get(i);
  //   double val = ((RooRealVar *)row->find("ctau3D"))->getVal();
  //   if (val >= xmin && val <= xmax)
  //     hTemp->Fill(val);
  // }

  // int rebinFactor = 1;
  // const double maxAllowed = 1e4; // 최대 허용 peak
  // double peak = hTemp->GetMaximum();

  // while (peak > maxAllowed && hTemp->GetNbinsX() / rebinFactor > 5)
  // {
  //   rebinFactor *= 2;
  //   TH1D *hRebinned = (TH1D *)hTemp->Rebin(rebinFactor, Form("hTemp_rebinned_%d", rebinFactor));
  //   peak = hRebinned->GetMaximum();
  //   delete hTemp;
  //   hTemp = hRebinned;
  // }

  // const int finalBins = hTemp->GetNbinsX();
  // RooBinning rb(finalBins, xmin, xmax);
  // std::cout << "[AutoBinning] Final bins = " << finalBins
  //           << ", peak = " << peak
  //           << ", rebinFactor = " << rebinFactor << std::endl;

  // delete hTemp;

  RooBinning rb(300, -0.5, 2);
  ...
  RooPlot *tframe1 = ws->var("ctau3D")->frame(Bins(rb.numBins()));
  double avgBinWidth = rb.averageBinWidth();
  tframe1->GetYaxis()->SetTitle(Form("Counts / (%.0f #mum)", avgBinWidth * 1000));
  tframe1->GetYaxis()->CenterTitle(1);
  redDataSB->plotOn(tframe1, DataError(RooAbsData::SumW2), Name("Data"));
  ...
  // --- pull ---
  RooHist *hpullsb = tframe1->pullHist("Data", "sumAll", true);

  int nFitPar = fitCt_Bkg->floatParsFinal().getSize();
  int nFullBins = 0;
  double chi2 = 0.0;

  // Loop over all points in pull histogram
  for (int i = 0; i < hpullsb->GetN(); ++i)
  {
    double x, y;
    hpullsb->GetPoint(i, x, y);

    // Skip invalid (NaN or inf) points
    if (!std::isfinite(y))
      continue;
  ...
  double chi2ndf = (dof > 0) ? chi2 / dof : 0.0;
  ...
  *Dof_side_t = dof;
  ...
  sprintf(reduceDS, "#chi^{2}/ndf = %.2f/%d", chi2, dof);
...
void drawFinalMass(RooWorkspace *ws, RooDataSet *redDataCut, float NSigNP_fin, float NBkg_fin,
                   RooFitResult *fitMass,
                   double *UnNormChi2_mass_t, int *nFitParam_mass_t, int *nFullBinsPull_mass_t,
                   int *Dof_mass_t, double *Chi2_mass_t)
{
  // ---- Labels ----
  TLatex *t = new TLatex();
  t->SetNDC();
  ...
  // ============================ BINNING (FAST & ROBUST) ============================
  const double xmin = 2.6, xmax = 3.5;
  // const int initBins = 300;        // starting bins
  // const double targetPeak = 1.0e4; // desired peak ~ 10k
  // const int minBins = 80, maxBins = 500;

  // // 1) measure peak with a temporary uniform binning
  // RooBinning rb0(initBins, xmin, xmax);
  // TH1 *htemp = redDataCut->createHistogram("h_tmp_mass", *ws->var("mass"), Binning(rb0));
  // htemp->Sumw2();
  // double peak = htemp->GetMaximum();
  // if (!std::isfinite(peak) || peak <= 0)
  //   peak = targetPeak; // guard

  // // 2) scale number of bins to cap peak near target
  // int factor = std::max(1, (int)std::ceil(peak / targetPeak));
  // int nbins = std::clamp(initBins * factor, minBins, maxBins);

  RooBinning rb(600, xmin, xmax); // final uniform binning
  // cout << "\n=== Mass nbins: " << nbins << " ===\n";
  ws->var("mass")->setBinning(rb);  // optional: set on the variable

  // ============================ MAIN FRAME (PRETTY) ============================
  RooPlot *mframe = ws->var("mass")->frame();
  redDataCut->plotOn(mframe, DataError(RooAbsData::SumW2),
                     XErrorSize(0), MarkerSize(1),
                     Binning(rb), Name("FinalMassData"));

  double avgBinWidth = rb.averageBinWidth(); // in GeV
  mframe->GetYaxis()->SetTitle(Form("Counts / (%.0f MeV/c^{2})", avgBinWidth * 1000.0));
  mframe->GetXaxis()->SetTitle("m_{#mu#mu} (GeV/c^{2})");
  mframe->GetXaxis()->CenterTitle(1);
  mframe->GetYaxis()->CenterTitle(1);

  // total pdf + components (cosmetics only)
  ...
  // re-draw data on top (must keep same Binning!)
  redDataCut->plotOn(mframe, DataError(RooAbsData::SumW2),
                     XErrorSize(0), MarkerSize(1),
                     Binning(rb), Name("FinalMassData"));

  // nice y-range after plotting
  const double yMax = mframe->GetMaximum() * 1.3;
  mframe->SetMaximum(yMax);
  mframe->SetMinimum(0);

  // ============================ PULL / CHI2 (CLEAN FRAME) ============================
  RooPlot *mframe_pullsrc = ws->var("mass")->frame(Bins(rb.numBins())); // match bins
  redDataCut->plotOn(mframe_pullsrc, DataError(RooAbsData::SumW2),
                     Binning(rb), Name("FinalMassData"));
  ws->pdf("totPDF_PEE")->plotOn(mframe_pullsrc, Normalization(redDataCut->sumEntries(), RooAbsReal::NumEvent), Name("FinalMassPdf"));

  RooHist *hpullm = mframe_pullsrc->pullHist("FinalMassData", "FinalMassPdf", true);
  hpullm->SetName("hpullM");
  ...
  int nFullBins = 0;
  double chi2 = 0.0;
  for (int i = 0; i < hpullm->GetN(); ++i)
  {
    double x, y;
    hpullm->GetPoint(i, x, y);
    if (!std::isfinite(y))
      continue; // guard NaN/Inf
    chi2 += y * y;
    nFullBins++;
  }
  int dof = nFullBins - nFitParam;
  double chi2ndf = (dof > 0) ? chi2 / dof : 0.0;

  // store to locals
  double UnNormChi2_mass = chi2;
  int nFitParam_mass = nFitParam;
  int nFullBins_mass = nFullBins;
  int Dof_mass = dof;
  double Chi2_mass = chi2ndf;

  // ============================ DRAW CANVAS ============================
  TCanvas c1("c1", "The mass Canvas", 200, 10, 600, 750);
  c1.cd();

  TPad *padm1 = new TPad("padm1", "fit pad", 0.0, 0.3, 1.0, 1.0);
  TPad *padm2 = new TPad("padm2", "pull pad", 0.0, 0.0, 1.0, 0.3);
  ...
  mframe->Draw();
  ty->SetTextSize(0.040);
  sprintf(reduceDS, "#chi^{2}/ndf = %.2f/%d", Chi2_mass, Dof_mass);
  ...
  double mframemax = std::max(std::abs(mframepull->GetMinimum()), mframepull->GetMaximum());
  mframepull->SetMaximum(mframemax);
  mframepull->SetMinimum(-mframemax);
  ...
  // ============================ OUTPUTS ============================
  if (UnNormChi2_mass_t)
    *UnNormChi2_mass_t = UnNormChi2_mass;
  if (nFitParam_mass_t)
    *nFitParam_mass_t = nFitParam_mass;
  if (nFullBinsPull_mass_t)
    *nFullBinsPull_mass_t = nFullBins_mass;
  if (Dof_mass_t)
    *Dof_mass_t = Dof_mass;
  if (Chi2_mass_t)
    *Chi2_mass_t = Chi2_mass;
}
...
void drawFinalCtau(RooWorkspace *ws, RooDataSet *redDataCut, RooDataHist *binDataCtErr,
                   float NSigNP_fin, float NBkg_fin, float Bfrac_fin, float ErrBfrac_fin,
                   RooFitResult *fit2D, float lmin, float lmax,
                   double *UnNormChi2_time_t, int *nFitParam_time_t, int *nFullBinsPull_time_t,
                   int *Dof_time_t, double *Chi2_time_t)
{
  char reduceDS[512];
  std::string titlestr;
  // RooRealVar *ctVar = ws->var("ctau3D");
  ...
  // // 임시 히스토그램 생성
  // TH1D *hTemp = new TH1D("hTemp", "temporary histogram", initBins, xmin, xmax);
  // for (int i = 0; i < redDataCut->numEntries(); ++i)
  // {
  //   const RooArgSet *row = redDataCut->get(i);
  //   double val = ((RooRealVar *)row->find("ctau3D"))->getVal();
  //   if (val >= xmin && val <= xmax)
  //     hTemp->Fill(val);
  // }
  ...
  RooBinning rb(1000, -0.5, 2);
  double avgBinWidth = rb.averageBinWidth();

  // ---------------- Main frame (keep all components for figure) ----------------
  ...
  // Total PDF (named) for figure (display scale can be 1 event)

  // Components (keep for figure)
  // ws->pdf("totPDF_PEE")->plotOn(tframe, Components("MassCtBkg"), Name("MassCtBkg"), LineColor(kBlue - 2), LineWidth(5), LineStyle(7), ProjWData(RooArgList(*(ws->var("ctau3DErr"))), *binDataCtErr, kTRUE), Normalization(1, RooAbsReal::NumEvent), NumCPU(32));

  // ws->pdf("totPDF_PEE")->plotOn(tframe, Components("MassCtNP"), Name("MassCtNP"),LineColor(kPink - 6), LineStyle(12), ProjWData(RooArgList(*(ws->var("ctau3DErr"))), *binDataCtErr, kTRUE), Normalization(1, RooAbsReal::NumEvent), NumCPU(32));

  // ws->pdf("totPDF_PEE")->plotOn(tframe, Components("MassCtPR"), Name("MassCtPR"),LineColor(kGreen + 3), LineStyle(kDashDotted), ProjWData(RooArgList(*(ws->var("ctau3DErr"))), *binDataCtErr, kTRUE), Normalization(1, RooAbsReal::NumEvent), NumCPU(32));

  ws->pdf("totPDF_PEE")->plotOn(tframe, LineColor(kBlack), LineWidth(2), ProjWData(RooArgList(*(ws->var("ctau3DErr"))), *binDataCtErr, kTRUE), Normalization(1, RooAbsReal::NumEvent), NumCPU(32), Name("FinalTimePdf"));


  // Pull from clean frame (data first, pdf second)
  RooHist *hpulltot = tframe->pullHist("FinalTimeData", "FinalTimePdf", true);
  ...
  int nFullBins = 0;
  double chi2 = 0.0;
  for (int i = 0; i < hpulltot->GetN(); ++i)
  {
    double x, y;
    hpulltot->GetPoint(i, x, y);
    if (!std::isfinite(y))
      continue; // skip NaN/inf
    chi2 += y * y;
    nFullBins++;
  }
  int dof = nFullBins - nFitPar;
  double chi2ndf = (dof > 0) ? chi2 / dof : 0.0;

  // Local copies; assign to outputs at the very end
  double UnNormChi2_time = chi2;
  int nFitParam_time = nFitPar;
  int nFullBins_time = nFullBins;
  int Dof_time = dof;
  double Chi2_time = chi2ndf;

  // ---------------- Drawing (figure + pull panel) ----------------
  TCanvas *c2 = new TCanvas("c2", "Ctau Canvas", 200, 10, 600, 750);
  TPad *pad1 = new TPad("pad1", "fit", 0.0, 0.3, 1.0, 1.0);
  TPad *pad2 = new TPad("pad2", "pull", 0.0, 0.0, 1.0, 0.3);
  pad1->SetLeftMargin(0.14);
  pad1->SetRightMargin(0.03);
  pad1->SetTopMargin(0.075);
  pad1->SetBottomMargin(0);
  ...
  double tframemax = std::max(std::abs(tframepull->GetMinimum()), tframepull->GetMaximum());
  tframepull->SetMaximum(tframemax);
  tframepull->SetMinimum(-tframemax);
  tframepull->GetXaxis()->SetTitle("#font[12]{l}_{J/#psi} (mm)");
  tframepull->GetXaxis()->CenterTitle(1);
  tframepull->Draw();

  TLine *line1 = new TLine(lmin, 0, lmax, 0.); // use given range directly
  line1->SetLineStyle(7);
  line1->Draw();
  ...
  t2->SetTextAlign(22);
  t2->SetTextSize(0.035 * 3);
  sprintf(reduceDS, "#chi^{2}/ndf = %.1f/%d", Chi2_time, Dof_time);
  t2->DrawLatex(0.78, 0.86, reduceDS);

  c2->SaveAs("_timefit_Lin.pdf");
  pad1->cd();
  pad1->SetLogy(1);
  c2->SaveAs("_timefit_Log.pdf");

  // ---------------- Assign outputs (single place) ----------------
  if (UnNormChi2_time_t)
    *UnNormChi2_time_t = UnNormChi2_time;
  if (nFitParam_time_t)
    *nFitParam_time_t = nFitParam_time;
  if (nFullBinsPull_time_t)
    *nFullBinsPull_time_t = nFullBins_time;
  if (Dof_time_t)
    *Dof_time_t = Dof_time;
  if (Chi2_time_t)
    *Chi2_time_t = Chi2_time;
}

void fit2D_622_adaptive_bins()
{
  cout << "=== start fit2D_622_adaptive_bins() ===\n";
  TStopwatch time;
  time.Start();
  ...
  // mass bin
  ws->var("mass")->setBins(45); // default : bin/0.02 GeV
  ...
  // -----------------------------
  RooFitResult *fitCt_PRMC = ws->pdf("CtPR_PEE")->fitTo(*redPRMC, Range("promptMCfit"), SumW2Error(kTRUE), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))), Save(1), NumCPU(32));
  // EvalBackend("legacy")
  fitCt_PRMC->Print("v");
  ...
  fit2D = ws->pdf("totPDF_PEE")->fitTo(*redData, Minos(0), Save(1), SumW2Error(kTRUE), PrintEvalErrors(-1), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))), NumCPU(32));
  ...
  cout << "=== finish fit2D_622_adaptive_bins() ===\n";
  time.Stop();
