...
static const double CT_EDGES[] = {-0.500, -0.450, -0.400, -0.350, -0.300, -0.250, -0.200, -0.150, -0.100, -0.050, 0.000, 0.050, 0.100, 0.150, 0.200, 0.250, 0.300, 0.350, 0.400, 0.450, 0.500, 0.550, 0.600, 0.650, 0.700, 0.750, 0.800, 0.850, 0.900, 0.950, 1.000, 1.050, 1.100, 1.150, 1.200, 1.250, 1.300, 1.350, 1.400, 1.450, 1.500, 1.550, 1.600, 1.650, 1.700, 1.750, 1.800, 1.850, 1.900, 1.950, 2.000}; // widht 0.125 - 25 bins
static const int N_CT_BINS = (int)(sizeof(CT_EDGES) / sizeof(CT_EDGES[0])) - 1;

void ctau_fit_pT6p5_7p5(double ptLow = 6.5, double ptHigh = 7.5)
{
  ...
  ct.setRange("ctRange", ctMin, ctMax);
  ...
  RooRealVar sigma1("sigma1", "core width", 0.05, 0.005, 0.1);
  RooRealVar sigma12("sigma12", "sigma 1 vs 2", 1.5, 1.0, 3);
  ...
  // recursive fractions
  RooRealVar fG1("fG1", "frac of g1", 0.3, 0.0, 1.0);
  ...
  RooRealVar lamP1("lamP1", "", 0.01, 0.0001, 1);
  RooRealVar lamP12("lamP12", "l", 1.1, 1, 10);
  RooFormulaVar lamP2("lamP2", "@0*@1", {lamP1, lamP12});

  RooRealVar lamN1("lamN1", "", 0.01, 0.0001, 1);
  // RooRealVar lamP1N1("lamP1N1", "", 5, 1, 10);
  ...
  RooDecay decayN("decayN", "left  exp", ct, lamN1, Res, RooDecay::DoubleSided);

  RooRealVar fP1("fP1", "", 0.3, 0, 1);
  RooRealVar fN1("fN1", "", 0.4, 0, 1);

  // RooFormulaVar wD2("wD2", "(1-@0)*@1", RooArgList(fG1, fG2));     // = (1-fG1)*fG2
  // RooFormulaVar wD3("wD3", "(1-@0)*(1-@1)", RooArgList(fG1, fG2)); // = (1-fG1)*(1-fG2)
  RooAddPdf npModel("npModel", "", {decayN, decayP1, decayP2}, {fN1, fP1});

  // const double Ntot = hNsig->Integral(); // get total number of events in the hist. not used now

  // --- final model ---
  RooRealVar bFrac("bFrac", "The b-fraction", 0.18, 0.10, 0.5);
  RooFormulaVar fPrompt("fPrompt", "1-@0", bFrac);

  RooAddPdf model("model_tmp", "", RooArgList(Res, npModel), RooArgList(fPrompt));
  ...
  RooFitResult *fitCtau = model.chi2FitTo(dh, Save(true), Extended(), SumW2Error(true), PrintLevel(-1), PrintEvalErrors(-1), Strategy(2), IntegrateBins(1e-6), Verbose(false));
