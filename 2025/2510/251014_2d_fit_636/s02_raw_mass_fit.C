...
void s02_raw_mass_fit()
{
  cout << "=== start s02_raw_mass() ===\n";
  TStopwatch time;
  time.Start();
  ...
  // fit
  RooFitResult *fr = Tot.fitTo(*redPRMC, Extended(), Save(), PrintLevel(-1), PrintEvalErrors(-1));
  ...
  c.cd(2);
  fp->Draw();
  c.SaveAs("figs/raw_mass_fit.pdf");

  // print out fit result
  fr->Print("V");

  TFile output("roots/raw_mass_fit.root", "recreate");
  fr->Write("fitMass");
  output.Close();

  cout << "=== finish s02_raw_mass() ===\n";
  time.Stop();
