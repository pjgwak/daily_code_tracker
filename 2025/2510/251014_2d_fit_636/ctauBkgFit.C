...
void setWSRange(RooWorkspace *ws, float lmin, float lmax, float errmin, float errmax)
{
  ...
  ws->var("ctau3D")->setRange("sbFit", -lmin, lmax);
  // ws->var("ctau3D")->setRange("CBkgFit", lmin, lmax);
  ws->var("ctau3DErr")->setRange(errmin, errmax);
  ws->var("ctau3DErr")->setRange("sbFit", errmin, errmax);
...
void drawCtauSBPlots(RooWorkspace *ws, RooDataSet *redDataSB, RooDataHist *binDataCtErrSB, RooFitResult *fitCt_Bkg, float lmin, float lmax, double *UnNormChi2_side_t, int *nFitParam_side_t, int *nFullBinsPull_side_t, int *Dof_side_t, double *Chi2_side_t)
{
  ...
  ws->pdf("CtBkgTot_PEE")->plotOn(tframe1, ProjWData(RooArgList(*(ws->var("ctau3DErr"))), *redDataSB, kTRUE), NumCPU(16), Normalization(redDataSB->sumEntries(), RooAbsReal::NumEvent), LineStyle(7), Name("model"));
    ws->pdf("CtBkgTot_PEE")->plotOn(tframe1, LineColor(kMagenta), LineStyle(3), Normalization(redDataSB->sumEntries(), RooAbsReal::NumEvent));
  ...
  c11->SaveAs("conditional_test.png");
    ...
    // ws->pdf("CtBkgTot_PEE")->plotOn(tframe1, NumCPU(16), LineStyle(7));

  // ws->pdf("CtBkgTot_PEE")->plotOn(tframe1, ProjWData(RooArgList(*ws->var("ctau3DErr")), *binDataCtErrSB), LineStyle(7), Name("model"));
...
void drawFitCt_PRMC(RooWorkspace *ws, RooDataSet *redPRMC, RooFitResult *fitCt_PRMC)
{
  ...
  c_lin->cd(1);
  gPad->SetPad(0.0, 0.3, 1.0, 1.0); // 상단 패드
  frame->Draw();
  {
  ...
  c_lin->cd(2);
  gPad->SetPad(0.0, 0.0, 1.0, 0.3); // 하단 패드
  frame_pull->Draw();
...
void drawFitCt_NPMC(RooWorkspace *ws, RooDataSet *redNPMC, RooFitResult *fitCt_NPMC)
{
  ...
  RooPlot *frame = ctau3D->frame(Title("Fit result: CtBkgTot_PEE vs redNPMC"));

  // === 데이터 플롯 ===
  // ws->pdf("CtBkgTot_PEE")->plotOn(frame, Range("sbFit"), NormRange("sbFit"), Normalization(redNPMC->sumEntries(), RooAbsReal::NumEvent)); // 83816

  ws->pdf("CtBkgTot_PEE")->plotOn(frame, Normalization(1, RooAbsReal::NumEvent)); // 83816

  redNPMC->plotOn(frame);

  ws->pdf("CtBkgTot_PEE")->plotOn(frame,ProjWData(RooArgSet(*(ws->var("ctau3DErr"))), *redNPMC), LineColor(kRed), LineWidth(2), Normalization(1, RooAbsReal::NumEvent));



  
  // Normalization(100000, RooAbsReal::NumEvent)
  ...
  frame->Draw();
  c_lin->Draw();
  c_lin->SaveAs("fitCt_NPMC_lin.png");
}
...
void ctauBkgFit()
{
  ...
  // --- kinematics ---
  float ptLow = 12, ptHigh = 15;
  float yLow = 0, yHigh = 1.6;
  float massLow = 2.6, massHigh = 3.5;
  int cLow = 0, cHigh = 180;
  double ctMin = -0.5, ctMax = 2; // lmin
  float errmin = 0, errmax = 0.3;
  ...
  RooHistPdf errPdfBkg("errPdfBkg", "Error PDF bkg scaled", RooArgSet(*(ws->var("ctau3DErr"))), *binScaledBKG, 2);
  ws->import(errPdfBkg);
  ...
  RooFitResult *fitCt_Bkg = ws->pdf("CtBkgTot_PEE")->fitTo(*redDataSB, SumW2Error(false), Minos(0), Save(1), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))), Optimize(0), NumCPU(32), EvalBackend("legacy"), RecoverFromUndefinedRegions(1.), PrintEvalErrors(-1), PrintLevel(-1));
