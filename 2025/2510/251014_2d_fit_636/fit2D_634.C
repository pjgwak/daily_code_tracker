...
void fit2D_634()
{
  cout << "=== start fit2D_634() ===\n";
  TStopwatch time;
  time.Start();

  // === Kinematics and observables ranges ===
  // --- kinematics ---
  float ptLow = 6.5, ptHigh = 6.51;
  ...
  // // ---------- Signal: binSubtractedSIG -----------
  // RooHistFunc f_errSig("f_errSig", "f(dt) raw (sig)",
  //                      RooArgList(*ws->var("ctau3DErr")), *binSubtractedSIG);

  // // ∫ f_errSig(ws->var("ctau3DErr")) dd t  (full range; 필요시 Range("ws->var("ctau3DErr")Phys")로 바꾸세요)
  // RooAbsReal *I_sig = f_errSig.createIntegral(RooArgSet(*ws->var("ctau3DErr"))); // heap 소유
  // RooFormulaVar f_errSig_unit("f_errSig_unit", "@0/@1", RooArgList(f_errSig, *I_sig));

  // // PDF로 랩핑 (수치정규화는 이미 끝났으므로 빠름)
  // RooGenericPdf errPdfSig("errPdfSig", "Error PDF signal (HistFunc)",
  //                         "@0", RooArgList(f_errSig_unit));

  // ws->import(errPdfSig);
  // delete I_sig; // ws->import가 복제했으니 정리 OK

  // // ---------- Background: binScaledBKG -----------
  // RooHistFunc f_errBkg("f_errBkg", "raw (bkg)",
  //                      RooArgList(*ws->var("ctau3DErr")), *binScaledBKG);

  // RooAbsReal *I_bkg = f_errBkg.createIntegral(RooArgSet(*ws->var("ctau3DErr")));
  // RooFormulaVar f_errBkg_unit("f_errBkg_unit", "@0/@1", RooArgList(f_errBkg, *I_bkg));

  // RooGenericPdf errPdfBkg("errPdfBkg", "Error PDF bkg (HistFunc)",
  //                         "@0", RooArgList(f_errBkg_unit));
  ...
  auto dt = ws->var("ctau3DErr");
  auto bPos = ws->pdf("CtBkgPos");       // Decay::CtBkgPos(ctau3D, ..., CtPRRes, SingleSided)
  auto bNeg = ws->pdf("CtBkgNeg");       // Decay::CtBkgNeg(... Flipped)
  auto bDbl = ws->pdf("CtBkgDbl");       // Decay::CtBkgDbl(... DoubleSided)

  // --- 각 성분 × p(dt)  (Conditional: 성분이 dt에 '조건부')
  RooProdPdf CtBkgPos_PEE("CtBkgPos_PEE", "CtBkgPos × p(dt)",
                          RooArgSet(*bPos, *(ws->pdf("errPdfBkg"))),
                          Conditional(RooArgSet(*bPos), RooArgSet(*ws->var("ctau3DErr"))));
  RooProdPdf CtBkgNeg_PEE("CtBkgNeg_PEE", "Ct",
                          RooArgSet(*bNeg, *(ws->pdf("errPdfBkg"))),
                          Conditional(RooArgSet(*bNeg), RooArgSet(*ws->var("ctau3DErr"))));
  RooProdPdf CtBkgDbl_PEE("CtBkgDbl_PEE", "CtB)",
                          RooArgSet(*bDbl, *(ws->pdf("errPdfBkg"))),
                          Conditional(RooArgSet(*bDbl), RooArgSet(*ws->var("ctau3DErr"))));

  ws->import(CtBkgPos_PEE);
  ws->import(CtBkgNeg_PEE);
  ws->import(CtBkgDbl_PEE);

  // --- 기존 합: CtBkgTot = (CtPRRes + …) 구조는 삭제/미사용 권장
  //     새로: (CtBkgPos_PEE ⊕ CtBkgNeg_PEE ⊕ CtBkgDbl_PEE)
  //     기존 frac 파라미터 재사용 (없으면 factory로 새로 만드세요)

  // 예) 기존과 동일한 분수 이름을 재사용할 때
  auto f1 = ws->var("fracCtBkg1"); // 0.3, 0.01..0.99
  auto f2 = ws->var("fracCtBkg2"); // 0.1, 0.01..0.99
  if (!f1 || !f2)
  {
    ::Error("makePEE", "missing fractions fracCtBkg1/2");
    return;
  }

  // 1단: POS vs NEG
  RooAddPdf CtBkgSum1_PEE("CtBkgSum1_PEE", "",
                          RooArgList(*ws->pdf("CtBkgPos_PEE"), *ws->pdf("CtBkgNeg_PEE")),
                          RooArgList(*f1));
  // 2단: (POS/NEG) vs DBL
  RooAddPdf CtBkgTot_PEE("CtBkgTot_PEE", "",
                         RooArgList(CtBkgSum1_PEE, *ws->pdf("CtBkgDbl_PEE")),
                         RooArgList(*f2));
  ...
  // [5] ctau bkg fit
  // ----------------
  auto dhDataSB = new RooDataHist("dhDataSB", "", RooArgSet(*ctau3D, *ctau3DErr), *redDataSB);
  ...
  RooFitResult *fitCt_Bkg = ws->pdf("CtBkgTot_PEE")->fitTo(*redDataSB, ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))));

  // RooFitResult *fitCt_Bkg = ws->pdf("CtBkgTot_PEE")->fitTo(*redDataSB, SumW2Error(kTRUE), Save(1), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))), PrintEvalErrors(-1), PrintLevel(-1), NumCPU(32));
  // EvalBackend("legacy"),
  ...
  // dhData->plotOn(fMass, Binning(120), DataError(RooAbsData::SumW2));
  // ws->pdf("totPDF_PEE")->plotOn(fMass, ProjWData(RooArgSet(*ctau3D, *ctau3DErr), *dhData), LineColor(kBlue), Normalization(dhData->sumEntries(), RooAbsReal::NumEvent));
  // fMass->Draw();
  ...
  // dhData->plotOn(f_ctau3D, Binning(50), DataError(RooAbsData::SumW2), Name("h_data_ctau3D"));
  // ws->pdf("totPDF_PEE")->plotOn(f_ctau3D, Range(-0.5, 2.0), LineColor(kBlue), ProjWData(RooArgSet(*mass, *ctau3DErr), *dhData), Normalization(dhData->sumEntries(), RooAbsReal::NumEvent), Name("model_ctau3D"));
  // f_ctau3D->Draw();
  ...
  // fPull_ctau3D->GetYaxis()->SetRangeUser(-5, 5);
  // fPull_ctau3D->GetXaxis()->SetLimits(-0.5, 2.0);
  // fPull_ctau3D->Draw();
