...
#include "pp_run2_pt12p15_y0_1p6.C"
...
void defineCtauRes(RooWorkspace *ws, FitConfigCtauRes &cfg);
void drawCtauFit(RooWorkspace *ws, RooRealVar &ctau3DRes, RooAbsPdf &model, RooAbsData &data, const FitConfigCtauRes &cfg, const RooFitResult *fitResult, int nBins = 80, const char *outPrefix = "ctau_res");

void ctau_mc_res_fit(const char *configFile = "cfgs/pp_run2_pt6p5_9_y0_1p6.C")
{
  gROOT->ProcessLineSync(Form(".x %s", configFile));
  FitConfigCtauRes cfg = getConfigCtauRes();
  ...
  // observable
  auto ctau3DRes = new RooRealVar("ctau3DRes", "", cfg.ctResLow, cfg.ctResHigh, "L_{J/#psi} [mm]");
  ctau3DRes->setRange("fitRange", cfg.ctResLow, cfg.ctResHigh);
  RooArgSet obs(*ctau3DRes);
  ...
  // === build model ===
  defineCtauRes(ws, cfg);

  // --- perform fit ---
  auto fitResult = ws->pdf("resModel")->fitTo(*dsReduced, Save(), Optimize(0), Range("fitRange"), SumW2Error(hasWeight), Offset(), NumCPU(32), EvalBackend("legacy"), RecoverFromUndefinedRegions(2.0), PrintEvalErrors(-1), PrintLevel(-1));

  // ------------------

  // === draw plots ===
  cout << "\n--- draw plots --- \n";
  drawCtauFit(ws, *ws->var("ctau3DRes"), *ws->pdf("resModel"), *dsReduced, cfg, fitResult, 80, "ctau_res");

  // === save results ===
  cout << "\n--- save results ---\n";
  TFile fout(Form("%s/ctau_res_pT%.1f_%.1f_y%.1f_%.1f.root", cfg.rootDir.c_str(), cfg.ptLow, cfg.ptHigh, cfg.yLow, cfg.yHigh), "RECREATE");
...
void defineCtauRes(RooWorkspace *ws, FitConfigCtauRes &cfg)
{
  ...
  if (cfg.ctauResPdf == "Gauss1") {
    ...
    // --- components ---
    ws->factory("GaussModel::g1(ctau3DRes,mu0,sigma1)");

    // --- 3-Gauss resolution model ---
    // to keep the name consistently
    ws->factory("GaussModel::resModel(ctau3DRes,mu0,sigma1)");
    ...
    // --- components
    ws->factory("GaussModel::g1(ctau3DRes,mu0,sigma1)");
    ws->factory("GaussModel::g2(ctau3DRes,mu0,sigma2)");
    ...
    ws->factory("GaussModel::g1(ctau3DRes,mu0,sigma1)");
    ws->factory("GaussModel::g2(ctau3DRes,mu0,sigma2)");
    ws->factory("GaussModel::g3(ctau3DRes,mu0,sigma3)");
...
void drawCtauFit(RooWorkspace *ws, RooRealVar &mass, RooAbsPdf &model, RooAbsData &data, const FitConfigCtauRes &cfg, const RooFitResult *fitResult, int nBins, const char *outPrefix)
{
  // --- small local helpers ---
  ...
  const bool wantGaus = (cfg.ctauResPdf == "DCBGauss");
