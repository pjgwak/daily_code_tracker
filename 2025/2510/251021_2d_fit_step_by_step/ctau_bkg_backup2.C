...
void ctau_bkg(double ptLow = 6.5, double ptHigh = 7.5)
{
  ...
  auto getRooVal = [](TFile& f, const char* name)->double {
    if (auto v = (RooRealVar*)f.Get(name)) return v->getVal();
    return 0.0;
  };

  ctLow  = getRooVal(fInData, "ctLow");
  ctHigh = getRooVal(fInData, "ctHigh");
  errLow = getRooVal(fInData, "errLow");
  errHigh= getRooVal(fInData, "errHigh");
  ...
  RooAddPdf model("model", "Total Decay (L/R/M with ctRes)",
                  RooArgList(ctRes_PEE, CtRight_PEE, CtLeft_PEE, CtMid_PEE),
  ...
  RooFitResult *fitRes = model.fitTo(*redData, Save(), PrintLevel(0), PrintEvalErrors(-1), NumCPU(1), EvalBackend("legacy"), RecoverFromUndefinedRegions(1.5), ConditionalObservables(RooArgSet(*ctau3DErr)));
