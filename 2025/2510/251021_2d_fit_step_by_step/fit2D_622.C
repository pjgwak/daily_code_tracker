...
void fit2D_622()
{
  ...
  // RooRealVar fCB1("fCB1","frac of CB1", 0.6, 0.0, 1.0);
  RooAddPdf DCB("DCB","Double Crystal Ball", RooArgList(CB1, CB2), RooArgList(fCB1));
  ...
  if (isSkipRawMass && !gSystem->AccessPathName(Form("%s/raw_mass_fit_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitRawMass = massFitModel.fitTo(*redData_woCtErr, Offset(true), Range("massRange"), Save(), Extended(), PrintLevel(-1), Strategy(2), PrintEvalErrors(-1), NumCPU(32)); //, EvalBackend("legacy")
  ...
  if (isSkipMass && !gSystem->AccessPathName(Form("%s/mass_fit_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitMass = massFitModel.fitTo(*redData_woCtErr, Offset(true), Range("massRange"), Save(), Extended(), PrintLevel(-1), Strategy(2), PrintEvalErrors(-1), NumCPU(32)); //, EvalBackend("legacy")
  ...
  if (isSkipRes && !gSystem->AccessPathName(Form("%s/ctau_res_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitRes = ctResFitModel.fitTo(*redPRMC, Range("ctResRange"), Offset(true), Save(), PrintLevel(-1), Strategy(2), PrintEvalErrors(-1), NumCPU(32)); //, EvalBackend("legacy"),  Range("massRange"),
  }
  ...
  RooRealVar resSigma1("resSigma1", "resolution sigma1", 0.05, 0.001, 1);
  ...
  if (isSkipBkg && !gSystem->AccessPathName(Form("%s/ctau_bkg_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitBkg = ctBkgFitModel.fitTo(*redDataSB, Save(), Optimize(0), Offset(), NumCPU(32),PrintEvalErrors(-1), PrintLevel(-1), RooFit::ExternalConstraints(resSigma1_constrain));
  fitBkg->Print("V");
  ...
  if (isSkipTrue && !gSystem->AccessPathName(Form("%s/ctau_true_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitTrue = ctTrueFitModel.fitTo(*redNPMC, Save(), Optimize(0), Offset(), NumCPU(32),  PrintEvalErrors(-1), PrintLevel(-1));
  fitTrue->Print("V");
  ...
  if (isSkipFinal && !gSystem->AccessPathName(Form("%s/final_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitFinal = finalFitModel.fitTo(*redData, Save(), Offset(), NumCPU(32),  PrintEvalErrors(-1), PrintLevel(-1));
  fitFinal->Print("V");
  ...
  output_current_time();
  {
    ...
    RooDataHist hProj("hProj","", RooArgList(*ctau3DErr, *ctau3D), *redData);

    // fr->SetNbins(25);
    redData->plotOn(fr, DataError(RooAbsData::SumW2), Name("data"));
    // finalFitModel.plotOn(fr, Name("model"), NumCPU(16), Precision(1));
    // RooNumIntConfig* intConfig = RooAbsReal::defaultIntegratorConfig();
    // intConfig->setEpsRel(1.E-5);
    // intConfig->getConfigSection("RooAdaptiveIntegratorND").setRealValue("maxEval2D", 1000);
...
//   auto makePEE = [&](const char *name, RooAbsPdf *src, RooWorkspace *ws)
//   {
//     RooProdPdf tmp(Form("%s_PEE", name), Form("%s * P(err)", name),
//                    RooArgSet(*ws->pdf("errPdfBkg")),
//                    RooFit::Conditional(*src, RooArgList(*ws->var("ctau3D"))));
//     ws->import(tmp);
//   };
...
//   RooFitResult *fitCt_Bkg = ws->pdf("CtBkgTot_PEE2")->fitTo(*redDataSB, SumW2Error(kTRUE), Save(1), NumCPU(32), PrintEvalErrors(-1), Strategy(2), ConditionalObservables(RooArgSet(*(ws->var("ctau3DErr")))), PrintLevel(0), EvalBackend("legacy"));
...
//   // ws->function("sigmaPRResW")->setConstant(kTRUE); // test

//   ws->var("fracRes")->setConstant(kTRUE); // test

//   drawCtauSBPlots(ws, redDataSB, binDataCtErrSB, fitCt_Bkg, -ctLow, ctHigh, &UnNormChi2_side, &nFitParam_side, &nFullBinsPull_side, &Dof_side, &Chi2_side);
