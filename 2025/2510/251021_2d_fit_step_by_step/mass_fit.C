...
void mass_fit(double ptLow = 6.5, double ptHigh = 7.5)
{
  ...
  RooRealVar mean("mean", "mean", 3.096, 3.090, 3.13);
  ...
  // --- fit ---
  cout << "\n=== Fit ===\n";
  RooFitResult *fitMass = model.fitTo(*redData, Range("massRange"), Save(), Extended(), PrintLevel(-1), Verbose(false), PrintEvalErrors(-1), NumCPU(32), EvalBackend("legacy"), Strategy(2));
  ...
  // --- save ---
  TFile f(Form("%s/mass_fit_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), "RECREATE");
