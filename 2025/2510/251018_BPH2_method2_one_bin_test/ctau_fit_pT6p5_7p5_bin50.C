...
void ctau_fit_pT6p5_7p5_bin50(double ptLow = 6.5, double ptHigh = 7.5)
{
  ...
  hNsig->Sumw2(); // can skip here using SetBinError()
  ...
  RooRealVar sigma1("sigma1", "core width", 0.01, 0.001, 0.2);
  ...
  // recursive fractions
  RooRealVar fG1("fG1", "frac of g1", 0.9, 0.7, 1.0);
  ...
  RooRealVar lamP1("lamP1", "", 0.02, 0.001, 1);
  RooRealVar lamP12("lamP12", "l", 1.1, 1, 5);
  ...
  RooAddPdf model("model", "", RooArgList(npModel, Res), RooArgList(bFrac));
  ...
  RooFitResult *fitCtau = model.chi2FitTo(dh, Save(true), SumW2Error(true), PrintLevel(-1), PrintEvalErrors(-1), Strategy(2), IntegrateBins(1e-4), Verbose(false));

  mean.setConstant(true);
  fitCtau = model.chi2FitTo(dh, Save(true), SumW2Error(true), PrintLevel(-1), PrintEvalErrors(-1), Strategy(2), IntegrateBins(1e-4), Verbose(false));
  ...
  cout << "\n=== finish ctau_fit_pT6p5_7p5_bin50() ===\n";
  time.Stop();
