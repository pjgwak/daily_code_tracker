...
void MassFit::buildModel()
{
  ...
  // RooCBShape *cb_1_A = new RooCBShape("cb_1_A", "cystal Ball", *(ws->var("mass")), mean, sigma_1_A, alpha_1_A, n_1_A);
  // RooAddPdf *pdfMASS_Jpsi;
  // RooCBShape *cb_2_A = new RooCBShape("cb_2_A", "cystal Ball", *(ws->var("mass")), mean, sigma_2_A, alpha_2_A, n_2_A);

  // combine signal models
  ...
  // else
  //   ws->factory("RooChebychev::pdfMASS_bkg(mass, {sl1})");
  ...
  // // build fit model
  // RooAddPdf *pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot", "Jpsi + Bkg", RooArgList(*pdfMASS_Jpsi, *ws->pdf("pdfMASS_bkg")), RooArgList(*N_Jpsi, *N_Bkg));
