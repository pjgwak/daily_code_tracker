...
// ----------
double ptLow = 30, ptHigh = 50, yLow = 0, yHigh = 1.6, massLow = 2.6, massHigh = 3.5;
double ctMin = -0.5, ctMax = 2;
double errMin = 0.02, errMax = 0.15;
...
for (int i = 0; i < nBins; i++) {
  ...
  RooRealVar sigma1("sigma1", "resolution sigma1", 0.08, 0.001, 0.1);
...
auto decayModel = new RooAddPdf("decayModel", "All decay components",
  ...
  fullModel.plotOn(frame, LineColor(kRed), LineWidth(2), Name("model"), NumCPU(16), Precision(-1)); //Normalization(80742, RooAbsReal::NumEvent)
