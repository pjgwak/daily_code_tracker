...
void mc_mass_center_fit()
{
  ...
  RooAddPdf model("sig", "signal = CBleft ⊕ CBright ⊕ Gauss",
  ...
  // --- perform fit ---
  auto fitResult = model.fitTo(*ds_red, Save(), Range("fitRange"), SumW2Error(hasWeight), Offset(true), PrintLevel(-1), NumCPU(30), Warnings(kFALSE), Verbose(kFALSE));
  ...
  if (massVar)
  {
    ...
    c_mass.SaveAs("figs_region6_test/mc_mass_center.png");
  }

  fitResult->Print();
  cout << "\n chi2/ndf = " << chi2ndf << endl;

  TFile fout("fitResult_mc_mass.root", "RECREATE"); // 저장할 파일
  fitResult->Write("fitResult");                 // RooFitResult 객체 저장
  model.Write();       // (선택) 모델 전체 저장
  fout.Close();
