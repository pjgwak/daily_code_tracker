...
void ctau_side_fit()
{
  ...
  std::string comp = "Side", region = "LSB";
  ...
  ctau3DVar->setRange(0.1, 0.5); // Change PDF's range - Need.
  ctau3DVar->setRange("fitRange", 0.1, 0.5);
  ...
  // RooRealVar tau1("tau1", "lifetime1", 0.05, 0.001, 10.0);
  // RooRealVar tau2("tau2", "lifetime2", 0.20, 0.001, 10.0);
  // RooRealVar tau3("tau3", "lifetime3", 1.00, 0.001, 10.0);

  // // --- coefficients c = -1/tau ---
  // RooFormulaVar c1("c1", "-1.0/@0", RooArgList(tau1));
  // RooFormulaVar c2("c2", "-1.0/@0", RooArgList(tau2));
  // RooFormulaVar c3("c3", "-1.0/@0", RooArgList(tau3));

  // // --- exponentials ---
  // RooExponential e1("e1", "exp with tau1", *ctau3DVar, c1);
  // RooExponential e2("e2", "exp with tau2", *ctau3DVar, c2);
  // RooExponential e3("e3", "exp with tau3", *ctau3DVar, c3);

  // // --- fractions ---
  // // f1: fraction of e1
  // // f2: fraction of e2
  // // e3 fraction은 자동으로 (1 - f1 - f2)로 결정됨
  // RooRealVar f1("f1", "frac fast", 0.3, 0.0, 1.0);
  // RooRealVar f2("f2", "frac medium", 0.3, 0.0, 1.0);

  // // --- 3-exponential mixture ---
  // RooAddPdf expo3(
  //     "expo3", "f1*e1 + f2*e2 + (1-f1-f2)*e3",
  //     RooArgList(e1, e2, e3),
  //     RooArgList(f1, f2) // fraction 리스트 (마지막은 자동 보정)
  // );
  ...
  if (ctau3DVar)
  {
    ...
    c_ctau.SaveAs(Form("figs_region6_test/ctau3D_%s_%s.png", comp.c_str(), region.c_str() ));
  }

  fitResult->Print();
  cout << "\n chi2/ndf = " << chi2ndf << endl;

  TFile fout(Form("fitResult_ctau_side_%s.root", region.c_str()), "RECREATE");
