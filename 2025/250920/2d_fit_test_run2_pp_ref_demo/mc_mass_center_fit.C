...
void mc_mass_center_fit()
{
  ...
  // === perform fit ===
  auto fitResult = model.fitTo(*ds_red, Save(), Range("fitRange"), SumW2Error(hasWeight), Offset(true), PrintLevel(-1), Warnings(kFALSE), Verbose(kFALSE), NumCPU(32), EvalBackend("legacy"));
  ...
  massFrame->SetMaximum(ymax * 100.0);
