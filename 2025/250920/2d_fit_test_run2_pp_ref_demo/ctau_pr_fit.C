...
void ctau_pr_fit()
{
  ...
  auto *ctRes = dynamic_cast<RooRealVar *>(data->get()->find("ctau3DRes")); // PR MC
  ...
  RooRealVar *fg1Var = (RooRealVar *)params.find("fg1"); // not fixed
  RooRealVar *fg2Var = (RooRealVar *)params.find("fg2"); // not fixed
  ...
  RooRealVar fg1("fg1", "fg1", fg1Var->getVal(), 0.01, 1);
  ...
  RooRealVar Nnp("Nnp", "yield nonprompt", 2.8221e+04, 5000, 100000);
  ...
  auto fitResult = model.fitTo(*dh, Extended(kTRUE), PrintLevel(-1), Save(), Offset(true), NumCPU(32), EvalBackend("legacy"), Strategy(2)); //, BatchMode(true)
