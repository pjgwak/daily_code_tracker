...
void ctau_side_fit(string region = "RSB")
{
  ...
  // --- region6 cuts ---
  double ctMin = -4, ctMax = 10;


  const TString cutPR = "(abs(ctau3D) < 0.05)";
  const TString cutNP = "(ctau3D >= 0.10 && ctau3D <= 0.80)";
  const TString cutCtauFull = Form("(ctau3D >= %.2f && ctau3D <= %.2f)", ctMin, ctMax);
  ...
  // variables
  RooRealVar *ctau3D = dynamic_cast<RooRealVar *>(ds_red->get()->find("ctau3D"));
  if (!ctau3D)
  ...
  RooRealVar *ratio_tom = new RooRealVar("s_{to}", "ratio", 1, 0.01, 10); // ratio of what?
  RooRealVar *Sigma0_tom = new RooRealVar("#sigma_{t0}", "proper time sigma", 0.1, 0.0001, 0.2);
  // per event err??
  ...
  RooRealVar tau_left("tau_left", "scale factor left", 10.0, 0.1, 100.0);
  ...
  // RooRealVar r_mid("r_mid", "scale factor middle", 3, 0.01, 10.0);
  ...
  RooRealVar f_right("f_right", "fraction middle", 0.3, 0.0, 1.0);
  ...
  auto fitResult = model->fitTo(*ds_red, Save(), Range("fitRange"), SumW2Error(hasWeight), Offset(true), Strategy(2),
                                NumCPU(32),
                                EvalBackend("legacy"),
