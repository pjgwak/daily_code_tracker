...
void final_2d_fit2()
{
  ...
  // --- 공통 평균 ---
  RooRealVar mean("mean", "mean of CB", 3.096, 3.09, 3.1);

  // === CB1 ===
  RooRealVar sigma1("sigma1", "CB1 sigma", 0.02, 0.01, 0.05);
  RooRealVar alpha1("alpha1", "CB1 alpha", 1.5, 0.1, 10.0);
  RooRealVar n1("n1", "CB1 n", 3.0, 1.0, 5.0);

  RooCBShape cb1("cb1", "Crystal Ball 1",
                 mass, mean, sigma1, alpha1, n1);

  // === CB2 ===
  RooRealVar sigma2("sigma2", "CB2 sigma", 0.02, 0.01, 0.05);
  RooRealVar alpha2("alpha2", "CB2 alpha", -1.5, -10.0, -0.1);
  RooRealVar n2("n2", "CB2 n", 3.0, 1.0, 5.0);

  RooCBShape cb2("cb2", "Crystal Ball 2",
                 mass, mean, sigma2, alpha2, n2);

  // === Gaussian ===
  RooRealVar sigmaG("sigmaG", "Gaussian sigma", 0.02, 0.01, 0.09);
  ...
  RooRealVar s1("s1", "Chebychev coefficient 0", 0.01, -1.0, 1.0);
  RooRealVar s2("s2", "Chebychev coefficient 1", 0.01, -1.0, 1.0);
  ...
  RooRealVar f1_ctau("f1_ctau", "frac G1 within (G1+G2)",
                     0.5, 0.0, 1.0);

  // (g1+g2) vs g3의 분할 비율
  RooRealVar f2_ctau("f2_ctau", "frac (G1+G2) within total",
                     0.7, 0.0, 1.0);
  ...
  RooAddModel resModel_ctau1("resModel_ctau1", "3-Gauss Resolution ctau",
                            RooArgList(g1_ctau, g2_ctau),
                            RooArgList(f1_ctau));
  RooAddModel resModel_ctau("resModel_ctau", "3-Gauss Resolution ctau",
                            RooArgList(resModel_ctau1, g3_ctau),
                            RooArgList(f2_ctau));
  ...
  RooAddPdf bkgModel_ctau("bkgModel_ctau", "Background ctau",
                          ...
                          RooArgList(fracRes, fracMid, fracRight), kTRUE);
  ...
  // ===== Total 2D model =====
  RooRealVar Nsig2D("Nsig2D", "yield signal 2D", 500000, 400000, 2000000);
  RooRealVar Nbkg2D("Nbkg2D", "yield background 2D", 200000, 50000, 1000000);
