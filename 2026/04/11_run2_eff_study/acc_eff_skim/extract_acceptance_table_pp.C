...
SumResult Integrate1D(const TH1D *num, const TH1D *den, double ptLow, double ptHigh)
{
  if (!num || !den)
...
void extract_acceptance_table_pp(const char *inputPath =
    "",
    ...
    bool verbose = false)
{
  ...
  const TString inputPathResolved =
      (inputPath && inputPath[0] != '\0')
          ? TString(inputPath)
          : inputDir + "/acc_pp2018_ppInput_isMC1_PR_ncollW0_genW1_ptW1.root";
  const TString outputPathResolved =
      (outputPath && outputPath[0] != '\0')
          ? TString(outputPath)
          : tableDir + "/acceptance_table_pp.tsv";
