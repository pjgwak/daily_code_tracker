...
static inline int ybin_abs(double yAbs)
{
...
void make_acc_from_tree(const char *inFileName = "roots/acceptance_Prompt_jpsi_GenOnly_ptWgtNo_sampleFlorain.root",
                        const char *outFileName = "roots/acc_maps.root")
{
  // === open input ===
  ...
  hAcc_y_pt->SetTitle("Acceptance; y; p_{T} (GeV)");
  ...
  // ================================
  // === (pT, |cosHX|) 2D acceptance split by |y|
  // ================================
  const int nCos = 10;
  ...
  for (int iy = 0; iy < 2; ++iy)
  {
    ...
    hAcc_cos_pt[iy]->SetTitle(Form("Acceptance (%s); |cos#theta_{HX}|; p_{T} (GeV)", ytag[iy]));
  }

  for (Long64_t i = 0; i < nDen; ++i)
  {
    tDen->GetEntry(i);
    int iy = ybin_abs(fabs(den_y));
    if (iy < 0)
      continue;
  ...
  // ================================
  // === (cosHX, phiHX) 2D acceptance split by |y|
  // ================================
  const int nPhi = 24;
  ...
  for (int iy = 0; iy < 2; ++iy)
  {
    ...
    hAcc_cos_phi_HX[iy]->SetTitle(Form("Acceptance (%s); cos#theta_{HX}; #phi_{HX} (rad)", ytag[iy]));
  }
  ...
  // ================================
  {
    TCanvas *c = new TCanvas("cAcc_cos_pt_byY_acc", "Acceptance: |cosHX| vs pT by |y|", 1200, 500);
    c->Divide(2, 1);
    for (int iy = 0; iy < 2; ++iy)
    {
      c->cd(iy + 1);
    ...
    c->SaveAs("figs/acc_cospt_byY.pdf");
    delete c;
  }

  {
    TCanvas *c = new TCanvas("cAcc_cos_phi_byY_acc", "Acceptance: cosHX vs phiHX by |y|", 1200, 500);
    c->Divide(2, 1);
    for (int iy = 0; iy < 2; ++iy)
    {
      c->cd(iy + 1);
    ...
    c->SaveAs("figs/acc_cosphi_byY.pdf");
    delete c;
  }
  ...
  cout << "[INFO] Acceptance histograms written to: " << outFileName << endl;
}
