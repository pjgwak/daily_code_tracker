...
void get_acc_file()
{
  ...
  string userLabel = "_sampleFlorain";

  // === read oniatree input ===
  TFile *rf = new TFile("/data/Oniatree/polarization/oniatree_5p36/2023_GenOnly/Oniatree_PromptJpsi_5p36TeV_GenOnly.root");
  ...
  TFile *outfile = new TFile(Form("roots/acceptance_Prompt_jpsi_GenOnly_ptWgt%s%s.root", ptWgtLabel.c_str(), userLabel.c_str()),"RECREATE");
  ...
  for(int i=0; i<nEvt/*nEvt*/; i++) {    
    ...
    for(int j = 0; j < Gen_QQ_size; j++) {
      ...
      // All dimuons - just for reference
      // hNocutY->Fill(y);
      // hNocutPt->Fill(pt);
      ...
      // === denominator cut: dimuon satisfying |y| < 2.4 ===
      ...
      // [Not] === apply pT weight - denominator ===
      // nominal, +err up, -err down
      // double wt1 = 1.0;
      // double wt2 = 1.0;
      // if (wtopt==1) {
      ...
      //   hDenPt_2021_Fory->Fill(pt,wt2);
      //   } 
      // if (1.6<=fabs(y) && fabs(y)<2.4 && pt > 3.5 && pt < 50) {
      //   hDenPt_2021_Fory_Int->Fill(1,wt2); hDenY_2021->Fill(y,wt2);
      // }
      // if (fabs(y)<1.6) {
      //   hDenPt_2021_midy->Fill(pt,wt1);
      // } 
      // if (fabs(y)<1.6 && pt > 6.5 && pt < 50) {
      //   hDenPt_2021_midy_Int->Fill(1,wt1); hDenY_2021->Fill(y,wt1);
      // }
