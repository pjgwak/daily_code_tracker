...
log_dir="logs/run_ctau_bins_${timestamp}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: bash run_ctau_bins.sh [--mc|--mass|--pr|--np|--bkg|--err|--2d] [--dry-run] [--max-jobs=N] [--max-log-mb=N] [--log-check-interval=N]" >&2
      exit 1
      ;;
  esac
done
