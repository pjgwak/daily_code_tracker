...
void makeMultiPanelCanvas(TCanvas*& canv, const Int_t columns,
  ...
  if (canv==0) {
   ...
   std::vector<std::vector<TPad*>> pad(columns, std::vector<TPad*>(rows, nullptr));
   
   std::vector<Float_t> Xlow(columns);
   std::vector<Float_t> Xup(columns);
   std::vector<Float_t> Ylow(rows);
   std::vector<Float_t> Yup(rows);
   Float_t PadWidth =
