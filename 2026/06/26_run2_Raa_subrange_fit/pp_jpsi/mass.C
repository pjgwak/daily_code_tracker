...
void mass(float ptLow = 6.5, float ptHigh = 9.0, float yLow = 0.0, float yHigh = 1.6, bool drawFromSavedFit = false, bool publish = false)
{
	...
	const char *DATA_ROOT = "/data/users/pjgwak/work/raa_pb18/run2_raa_pbpb2018/skimmedFiles/OniaRooDataSet_isMC0_JPsi_pp_y0.00_2.40_Effw0_Accw0_PtW1_TnP1_260303_root634.root";
	...
	else
	{
		...
		if (constraints.getSize() > 0)
			mass_result.reset(mass_pdf->fitTo(*data, Extended(), Save(), Strategy(2), PrintLevel(0), SumW2Error(isWeight), Minimizer("Minuit2", "migrad"), ExternalConstraints(constraints)));
		else
		...
		if (mass_result && (mass_result->status() != 0 || mass_result->covQual() < 2))
		{
			...
			if (constraints.getSize() > 0)
				mass_result.reset(mass_pdf->fitTo(*data, Extended(), Save(), Strategy(2), PrintLevel(0), SumW2Error(isWeight), ExternalConstraints(constraints)));
			else
