...
void draw_eff_pp(bool isNCollW = true, bool isGenW = true, bool isPtW = true, bool isTnPW = true)
{
  ...
  const std::vector<RapidityCase> rapidityCases = {
      {"hist_eff_rap0024", "rap0024", "|y| < 2.4, high-p_{T} selection", "|y|<2.4"},
      {"hist_eff_rap0006", "rap0006", "|y| < 0.6, high-p_{T} selection", "|y|<0.6"},
      {"hist_eff_rap0612", "rap0612", "0.6 < |y| < 1.2, high-p_{T} selection", "0.6<|y|<1.2"},
      {"hist_eff_rap1216", "rap1216", "1.2 < |y| < 1.6, high-p_{T} selection", "1.2<|y|<1.6"},
      {"hist_eff_rap1624", "rap1624", "1.6 < |y| < 2.4, low-p_{T} included", "1.6<|y|<2.4"},
  };
  ...
  for (const auto &rapCase : rapidityCases)
  {
    ...
    saveSinglePlot(hPr.get(), ptSingleDir + "/eff_pr_" + suffix + weightTag, "Prompt MC", label, "p_{T} (GeV/c)", "Efficiency");
    saveSinglePlot(hNp.get(), ptSingleDir + "/eff_np_" + suffix + weightTag, "Nonprompt MC", label, "p_{T} (GeV/c)", "Efficiency");
    saveOverlayPlot(hPr.get(), hNp.get(), ptOverlayDir + "/eff_pr_np_" + suffix + weightTag, label, "p_{T} (GeV/c)");

    ptSummaryStore.push_back(cloneDetached(hPr.get(), TString::Format("summary_pr_%s", rapCase.suffix)));
    ptSummaryStore.push_back(cloneDetached(hNp.get(), TString::Format("summary_np_%s", rapCase.suffix)));
    ptSummaryPairs.emplace_back(ptSummaryStore[ptSummaryStore.size() - 2].get(),
                                ptSummaryStore[ptSummaryStore.size() - 1].get());
  ...
  if (hPrY && hNpY)
  {
    ...
    saveSinglePlot(hPrY.get(), ySingleDir + "/eff_y_pr" + weightTag, "Prompt MC", "Integrated p_{T} selection", "|y|", "Efficiency");
    saveSinglePlot(hNpY.get(), ySingleDir + "/eff_y_np" + weightTag, "Nonprompt MC", "Integrated p_{T} selection", "|y|", "Efficiency");
    saveOverlayPlot(hPrY.get(), hNpY.get(), yOverlayDir + "/eff_y_pr_np" + weightTag, "Integrated p_{T} selection", "|y|");
  }
}
