...
void draw_eff_pt_cent_ptw_compare(bool isNCollW = true, bool isGenW = true, bool isTnPW = true)
{
  ...
  const std::vector<HistCase> histCases = {
      ...
      {"hist_eff_rap0024", "rap0024", "|y| < 2.4, 6.5 < p_{T} < 40 GeV/c", "p_{T} (GeV/c)", false},
      {"hist_eff_rap0006", "rap0006", "|y| < 0.6, 6.5 < p_{T} < 40 GeV/c", "p_{T} (GeV/c)", false},
      {"hist_eff_rap0612", "rap0612", "0.6 < |y| < 1.2, 6.5 < p_{T} < 40 GeV/c", "p_{T} (GeV/c)", false},
      {"hist_eff_rap1216", "rap1216", "1.2 < |y| < 1.6, 6.5 < p_{T} < 40 GeV/c", "p_{T} (GeV/c)", false},
      {"hist_eff_rap1624", "rap1624", "1.6 < |y| < 2.4, 3.5 < p_{T} < 40 GeV/c", "p_{T} (GeV/c)", false},
      {"hist_eff_pt_cent0010", "pt_cent0010", "0-10%, |y| < 2.4", "p_{T} (GeV/c)", false},
      {"hist_eff_pt_cent1030", "pt_cent1030", "10-30%, |y| < 2.4", "p_{T} (GeV/c)", false},
      {"hist_eff_pt_cent3090", "pt_cent3090", "30-90%, |y| < 2.4", "p_{T} (GeV/c)", false},
      {"hist_eff_pt_mid_cent0010", "pt_mid_cent0010", "0-10%, |y| < 1.6", "p_{T} (GeV/c)", false},
      {"hist_eff_pt_mid_cent1030", "pt_mid_cent1030", "10-30%, |y| < 1.6", "p_{T} (GeV/c)", false},
      {"hist_eff_pt_mid_cent3090", "pt_mid_cent3090", "30-90%, |y| < 1.6", "p_{T} (GeV/c)", false},
      ...
      {"hist_eff_pt_fwd_cent3090", "pt_fwd_cent3090", "30-90%, 1.6 < |y| < 2.4", "p_{T} (GeV/c)", false},
      {"hist_eff_cent_mid_stdpt", "cent_mid", "|y| < 1.6, 6.5 < p_{T} < 40 GeV/c", "Centrality", true},
      {"hist_eff_cent_fwd_stdpt", "cent_fwd", "1.6 < |y| < 2.4, 3.5 < p_{T} < 40 GeV/c", "Centrality", true},
      {"hist_eff_cent_mid_0180", "cent_mid_0180", "|y| < 1.6, cent 0-90% integrated", "Centrality", true},
      {"hist_eff_cent_fwd_0180", "cent_fwd_0180", "1.6 < |y| < 2.4, cent 0-90% integrated", "Centrality", true},
      {"hist_eff_cent_rap0024", "cent_rap0024", "|y| < 2.4, 6.5 < p_{T} < 40 GeV/c", "Centrality", true},
      {"hist_eff_cent_rap0006", "cent_rap0006", "|y| < 0.6, 6.5 < p_{T} < 40 GeV/c", "Centrality", true},
      {"hist_eff_cent_rap0612", "cent_rap0612", "0.6 < |y| < 1.2, 6.5 < p_{T} < 40 GeV/c", "Centrality", true},
      {"hist_eff_cent_rap1216", "cent_rap1216", "1.2 < |y| < 1.6, 6.5 < p_{T} < 40 GeV/c", "Centrality", true},
      {"hist_eff_cent_rap1624", "cent_rap1624", "1.6 < |y| < 2.4, 6.5 < p_{T} < 40 GeV/c", "Centrality", true},
      {"hist_eff_cent_rap1624_pt3565", "cent_rap1624_pt3565", "1.6 < |y| < 2.4, 3.5 < p_{T} < 6.5 GeV/c", "Centrality", true},
  };
  ...
  for (const auto &sample : samples)
  {
    ...
    for (const auto &histCase : histCases)
    {
      ...
      if (TString(histCase.histName) == "hist_eff_cent_mid_stdpt")
        fallbackName = "hist_eff_cent_mid";
      else if (TString(histCase.histName) == "hist_eff_cent_fwd_stdpt")
        fallbackName = "hist_eff_cent_fwd";

      std::unique_ptr<TH1D> hOff(loadHistWithFallback(fOff.get(), histCase.histName, fallbackName,
                                                      TString::Format("%s_%s_off", sample.shortLabel.Data(), histCase.suffix),
                                                      false));
      std::unique_ptr<TH1D> hOn(loadHistWithFallback(fOn.get(), histCase.histName, fallbackName,
    ...
    std::unique_ptr<TH1D> effFwdOff(loadHistWithFallback(fOff.get(), "hist_eff_cent_fwd_stdpt", "hist_eff_cent_fwd", sample.shortLabel + "_effFwdOff", false));
    std::unique_ptr<TH1D> effFwdOn(loadHistWithFallback(fOn.get(), "hist_eff_cent_fwd_stdpt", "hist_eff_cent_fwd", sample.shortLabel + "_effFwdOn", false));
    std::unique_ptr<TH1D> effHighOff(loadHist(fOff.get(), "hist_eff_cent_rap1624", sample.shortLabel + "_effHighOff", false));
    std::unique_ptr<TH1D> effHighOn(loadHist(fOn.get(), "hist_eff_cent_rap1624", sample.shortLabel + "_effHighOn", false));
    std::unique_ptr<TH1D> effLowOff(loadHist(fOff.get(), "hist_eff_cent_rap1624_pt3565", sample.shortLabel + "_effLowOff", false));
    std::unique_ptr<TH1D> effLowOn(loadHist(fOn.get(), "hist_eff_cent_rap1624_pt3565", sample.shortLabel + "_effLowOn", false));
    std::unique_ptr<TH1D> denHighOff(loadHist(fOff.get(), "hist_eff_den_cent_rap1624", sample.shortLabel + "_denHighOff", false));
    std::unique_ptr<TH1D> denHighOn(loadHist(fOn.get(), "hist_eff_den_cent_rap1624", sample.shortLabel + "_denHighOn", false));
    std::unique_ptr<TH1D> denLowOff(loadHist(fOff.get(), "hist_eff_den_cent_rap1624_pt3565", sample.shortLabel + "_denLowOff", false));
    std::unique_ptr<TH1D> denLowOn(loadHist(fOn.get(), "hist_eff_den_cent_rap1624_pt3565", sample.shortLabel + "_denLowOn", false));
    if (effFwdOff && effFwdOn && effHighOff && effHighOn && effLowOff && effLowOn &&
        denHighOff && denHighOn && denLowOff && denLowOn)
    {
      std::unique_ptr<TH1D> denTotOff(static_cast<TH1D *>(denHighOff->Clone(sample.shortLabel + "_denTotOff")));
      denTotOff->SetDirectory(nullptr);
      denTotOff->Add(denLowOff.get());
      std::unique_ptr<TH1D> denTotOn(static_cast<TH1D *>(denHighOn->Clone(sample.shortLabel + "_denTotOn")));
      denTotOn->SetDirectory(nullptr);
      denTotOn->Add(denLowOn.get());

      std::unique_ptr<TH1D> lowFracOff(makeFractionHist(denLowOff.get(), denTotOff.get(), sample.shortLabel + "_lowFracOff"));
      std::unique_ptr<TH1D> lowFracOn(makeFractionHist(denLowOn.get(), denTotOn.get(), sample.shortLabel + "_lowFracOn"));

      TString sampleTag = sample.shortLabel;
      sampleTag.ToLower();
      const TString debugPath = debugDir + "/eff_" + sampleTag + "_ptw_compare_cent_fwd_breakdown" +
                                Form("_ncollW%d_genW%d_tnpW%d",
                                     isNCollW ? 1 : 0, isGenW ? 1 : 0, isTnPW ? 1 : 0);
      saveFwdCentDebugPlot(effFwdOff.get(), effFwdOn.get(),
                           effHighOff.get(), effHighOn.get(),
                           effLowOff.get(), effLowOn.get(),
                           lowFracOff.get(), lowFracOn.get(),
                           debugPath, sample.displayLabel);
    }
  }
}
