...
void draw_eff_pt_cent(bool isNCollW = true, bool isGenW = true, bool isPtW = true, bool isTnPW = true)
{
  ...
  std::unique_ptr<TH1D> hPrCentMid(loadHistWithFallback(fPr.get(), "hist_eff_cent_mid_stdpt", "hist_eff_cent_mid", "hPrCentMid"));
  std::unique_ptr<TH1D> hPrCentFwd(loadHistWithFallback(fPr.get(), "hist_eff_cent_fwd_stdpt", "hist_eff_cent_fwd", "hPrCentFwd"));
  std::unique_ptr<TH1D> hNpCentMid(loadHistWithFallback(fNp.get(), "hist_eff_cent_mid_stdpt", "hist_eff_cent_mid", "hNpCentMid"));
  std::unique_ptr<TH1D> hNpCentFwd(loadHistWithFallback(fNp.get(), "hist_eff_cent_fwd_stdpt", "hist_eff_cent_fwd", "hNpCentFwd"));
  std::unique_ptr<TH1D> hPrCentMid0180(loadHist(fPr.get(), "hist_eff_cent_mid_0180", "hPrCentMid0180", false));
  std::unique_ptr<TH1D> hPrCentFwd0180(loadHist(fPr.get(), "hist_eff_cent_fwd_0180", "hPrCentFwd0180", false));
  std::unique_ptr<TH1D> hNpCentMid0180(loadHist(fNp.get(), "hist_eff_cent_mid_0180", "hNpCentMid0180", false));
  std::unique_ptr<TH1D> hNpCentFwd0180(loadHist(fNp.get(), "hist_eff_cent_fwd_0180", "hNpCentFwd0180", false));
  ...
  const std::vector<RapidityCase> rapidityCases = {
      {"hist_eff_rap0024", "rap0024", "|y| < 2.4, 6.5 < p_{T} < 40 GeV/c", "|y|<2.4"},
      {"hist_eff_rap0006", "rap0006", "|y| < 0.6, 6.5 < p_{T} < 40 GeV/c", "|y|<0.6"},
      {"hist_eff_rap0612", "rap0612", "0.6 < |y| < 1.2, 6.5 < p_{T} < 40 GeV/c", "0.6<|y|<1.2"},
      {"hist_eff_rap1216", "rap1216", "1.2 < |y| < 1.6, 6.5 < p_{T} < 40 GeV/c", "1.2<|y|<1.6"},
      {"hist_eff_rap1624", "rap1624", "1.6 < |y| < 2.4, 3.5 < p_{T} < 40 GeV/c", "1.6<|y|<2.4"},
  };
  ...
  ptSummaryLabels.push_back("1.6<|y|<2.4");

  for (const auto &rapCase : rapidityCases)
  {
    const TString histName = rapCase.histName;
    ...
    const TString suffix = rapCase.suffix;
    const TString label = rapCase.label;
    ...
    ptSummaryPairs.emplace_back(hPr.release(), hNp.release());
    ptSummaryLabels.push_back(rapCase.summaryLabel);
  }
  ...
  const std::vector<PtCentCase> ptCentCases = {
      {"pt_cent0010", "0-10%, |y| < 2.4", "0-10%"},
      {"pt_cent1030", "10-30%, |y| < 2.4", "10-30%"},
      {"pt_cent3090", "30-90%, |y| < 2.4", "30-90%"},
  };
  ...
  struct PtCentRapCase {
    const char *fileSuffix;
    const char *headerLabel;
  };
  const std::vector<PtCentRapCase> ptCentRapCases = {
      {"mid", "|y| < 1.6"},
      {"fwd", "1.6 < |y| < 2.4"},
  };
  for (const auto &rapCase : ptCentRapCases)
  {
    std::vector<std::pair<TH1D *, TH1D *>> rapSummaryPairs;
    std::vector<TString> rapSummaryLabels;
    std::vector<std::unique_ptr<TH1D>> rapSummaryStore;

    for (const auto &centCase : ptCentCases)
    {
      const TString histName = TString::Format("hist_eff_pt_%s_%s", rapCase.fileSuffix, centCase.suffix + 3);
      std::unique_ptr<TH1D> hPr(loadHist(fPr.get(), histName, TString::Format("%s_pr", histName.Data()), false));
      std::unique_ptr<TH1D> hNp(loadHist(fNp.get(), histName, TString::Format("%s_np", histName.Data()), false));
      if (!hPr || !hNp)
        continue;
      ...
      rapSummaryStore.push_back(std::move(hPr));
      rapSummaryStore.push_back(std::move(hNp));
      rapSummaryPairs.emplace_back(rapSummaryStore[rapSummaryStore.size() - 2].get(),
                                   rapSummaryStore[rapSummaryStore.size() - 1].get());
      rapSummaryLabels.push_back(centCase.summaryLabel);
    }

    if (!rapSummaryPairs.empty())
      saveSummaryPlot(rapSummaryPairs, rapSummaryLabels,
                      summaryPtDir + TString::Format("/eff_pt_pr_np_cent_summary_%s%s", rapCase.fileSuffix, weightTag.Data()),
                      "p_{T} (GeV/c)", rapCase.headerLabel);
  }
  ...
  if (hPrCentMid && hPrCentFwd && hNpCentMid && hNpCentFwd)
  {
    ...
    centSummaryLabels.push_back("mid");
    centSummaryPairs.push_back({hPrCentFwd.get(), hNpCentFwd.get()});
    centSummaryLabels.push_back("fwd");
  }
  else
  {
  ...
  if (hPrCentMid0180 || hPrCentFwd0180 || hNpCentMid0180 || hNpCentFwd0180)
  {
    const TString mid0180Label = "|y| < 1.6, cent 0-90% integrated";
    const TString fwd0180Label = "1.6 < |y| < 2.4, cent 0-90% integrated";
    if (hPrCentMid0180)
    {
      styleHist(hPrCentMid0180.get(), kAzure + 2, 24, 1.7);
      saveSinglePlot(hPrCentMid0180.get(), centPrDir + "/eff_cent_pr_mid_0180" + weightTag, "Prompt MC", mid0180Label, "Centrality", "Efficiency");
    }
    if (hPrCentFwd0180)
    {
      styleHist(hPrCentFwd0180.get(), kAzure + 2, 24, 1.7);
      saveSinglePlot(hPrCentFwd0180.get(), centPrDir + "/eff_cent_pr_fwd_0180" + weightTag, "Prompt MC", fwd0180Label, "Centrality", "Efficiency");
    }
    if (hNpCentMid0180)
    {
      styleHist(hNpCentMid0180.get(), kOrange + 7, 25, 1.7);
      saveSinglePlot(hNpCentMid0180.get(), centNpDir + "/eff_cent_np_mid_0180" + weightTag, "Nonprompt MC", mid0180Label, "Centrality", "Efficiency");
    }
    if (hNpCentFwd0180)
    {
      styleHist(hNpCentFwd0180.get(), kOrange + 7, 25, 1.7);
      saveSinglePlot(hNpCentFwd0180.get(), centNpDir + "/eff_cent_np_fwd_0180" + weightTag, "Nonprompt MC", fwd0180Label, "Centrality", "Efficiency");
    }
    if (hPrCentMid0180 && hNpCentMid0180)
      saveOverlayPlot(hPrCentMid0180.get(), hNpCentMid0180.get(), centCompareDir + "/eff_cent_pr_np_mid_0180" + weightTag, mid0180Label, "Centrality");
    if (hPrCentFwd0180 && hNpCentFwd0180)
      saveOverlayPlot(hPrCentFwd0180.get(), hNpCentFwd0180.get(), centCompareDir + "/eff_cent_pr_np_fwd_0180" + weightTag, fwd0180Label, "Centrality");
  }

  const std::vector<RapidityCase> centCases = {
      {"hist_eff_cent_rap0024", "cent_rap0024", "|y| < 2.4, 6.5 < p_{T} < 40 GeV/c", "|y|<2.4"},
      {"hist_eff_cent_rap0006", "cent_rap0006", "|y| < 0.6, 6.5 < p_{T} < 40 GeV/c", "|y|<0.6"},
      {"hist_eff_cent_rap0612", "cent_rap0612", "0.6 < |y| < 1.2, 6.5 < p_{T} < 40 GeV/c", "0.6<|y|<1.2"},
      {"hist_eff_cent_rap1216", "cent_rap1216", "1.2 < |y| < 1.6, 6.5 < p_{T} < 40 GeV/c", "1.2<|y|<1.6"},
      {"hist_eff_cent_rap1624", "cent_rap1624", "1.6 < |y| < 2.4, 6.5 < p_{T} < 40 GeV/c", "1.6<|y|<2.4"},
      {"hist_eff_cent_rap1624_pt3565", "cent_rap1624_pt3565", "1.6 < |y| < 2.4, 3.5 < p_{T} < 6.5 GeV/c", "1.6<|y|<2.4, low"},
  };
  for (const auto &centCase : centCases)
  {
    const TString histName = centCase.histName;
    std::unique_ptr<TH1D> hPr(loadHist(fPr.get(), histName, TString::Format("%s_pr", histName.Data()), false));
    std::unique_ptr<TH1D> hNp(loadHist(fNp.get(), histName, TString::Format("%s_np", histName.Data()), false));
    if (!hPr || !hNp)
      continue;
    ...
    const TString suffix = centCase.suffix;
    const TString label = centCase.label;
    saveSinglePlot(hPr.get(), centPrDir + "/eff_cent_pr_" + suffix + weightTag, "Prompt MC", label, "Centrality", "Efficiency");
    saveSinglePlot(hNp.get(), centNpDir + "/eff_cent_np_" + suffix + weightTag, "Nonprompt MC", label, "Centrality", "Efficiency");
    saveOverlayPlot(hPr.get(), hNp.get(), centCompareDir + "/eff_cent_pr_np_" + suffix + weightTag, label, "Centrality");
    centSummaryPairs.emplace_back(hPr.release(), hNp.release());
    centSummaryLabels.push_back(centCase.summaryLabel);
  }
  ...
  std::unique_ptr<TH1D> hPrY(loadHist(fPr.get(), "hist_eff_y", "hPrY", false));
  std::unique_ptr<TH1D> hNpY(loadHist(fNp.get(), "hist_eff_y", "hNpY", false));
  if (hPrY && hNpY)
  {
    const TString yLabel = "0-90%, 6.5 < p_{T} < 40 GeV/c";
    styleHist(hPrY.get(), kAzure + 2, 24, 1.7);
    styleHist(hNpY.get(), kOrange + 7, 25, 1.7);
    saveSinglePlot(hPrY.get(), yPrDir + "/eff_y_pr" + weightTag, "Prompt MC", yLabel, "|y|", "Efficiency");
    saveSinglePlot(hNpY.get(), yNpDir + "/eff_y_np" + weightTag, "Nonprompt MC", yLabel, "|y|", "Efficiency");
    saveOverlayPlot(hPrY.get(), hNpY.get(), yCompareDir + "/eff_y_pr_np" + weightTag, yLabel, "|y|");
  }
}
