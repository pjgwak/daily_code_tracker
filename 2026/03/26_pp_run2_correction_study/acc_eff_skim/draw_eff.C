...
void draw_eff(bool isNCollW = true, bool isGenW = true, bool isPtW = false, bool isTnPW = true)
{
  ...
  const TString effFigsDir = figsDir + "/eff";
  ...
  std::unique_ptr<TH1D> hPrCentMid(loadHistWithFallback(fPr.get(), "hist_eff_cent_mid_stdpt", "hist_eff_cent_mid", "hist_eff_cent_mid_pr"));
  std::unique_ptr<TH1D> hPrCentFwd(loadHistWithFallback(fPr.get(), "hist_eff_cent_fwd_stdpt", "hist_eff_cent_fwd", "hist_eff_cent_fwd_pr"));
  std::unique_ptr<TH1D> hNpCentMid(loadHistWithFallback(fNp.get(), "hist_eff_cent_mid_stdpt", "hist_eff_cent_mid", "hist_eff_cent_mid_np"));
  std::unique_ptr<TH1D> hNpCentFwd(loadHistWithFallback(fNp.get(), "hist_eff_cent_fwd_stdpt", "hist_eff_cent_fwd", "hist_eff_cent_fwd_np"));
  ...
  const TString midLabel = "|y| < 1.6, 6.5 < p_{T} < 50 GeV/c";
  const TString fwdLabel = "1.6 < |y| < 2.4, 3.5 < p_{T} < 50 GeV/c";
  ...
  const std::vector<PlotCase> ptCases = {
      {"rap0024", "|y| < 2.4, cent 0-90%", "|y|<2.4", "p_{T} (GeV/c)"},
      {"rap0006", "|y| < 0.6, cent 0-90%", "|y|<0.6", "p_{T} (GeV/c)"},
      {"rap0612", "0.6 < |y| < 1.2, cent 0-90%", "0.6<|y|<1.2", "p_{T} (GeV/c)"},
      {"rap1218", "1.2 < |y| < 1.8, cent 0-90%", "1.2<|y|<1.8", "p_{T} (GeV/c)"},
      {"rap1824", "1.8 < |y| < 2.4, cent 0-90%", "1.8<|y|<2.4", "p_{T} (GeV/c)"},
      {"pt_cent0010", "|y| < 2.4, 0-10%", "0-10%", "p_{T} (GeV/c)"},
      {"pt_cent1030", "|y| < 2.4, 10-30%", "10-30%", "p_{T} (GeV/c)"},
      {"pt_cent3090", "|y| < 2.4, 30-90%", "30-90%", "p_{T} (GeV/c)"},
  };
  ...
  const std::vector<PlotCase> centCases = {
      {"cent_rap0024", "|y| < 2.4, 6.5 < p_{T} < 50 GeV/c", "|y|<2.4", "Centrality"},
      {"cent_rap0006", "|y| < 0.6, 6.5 < p_{T} < 50 GeV/c", "|y|<0.6", "Centrality"},
      {"cent_rap0612", "0.6 < |y| < 1.2, 6.5 < p_{T} < 50 GeV/c", "0.6<|y|<1.2", "Centrality"},
      {"cent_rap1218", "1.2 < |y| < 1.8, 6.5 < p_{T} < 50 GeV/c", "1.2<|y|<1.8", "Centrality"},
      {"cent_rap1824", "1.8 < |y| < 2.4, 6.5 < p_{T} < 50 GeV/c", "1.8<|y|<2.4", "Centrality"},
      {"cent_rap1824_pt3565", "1.8 < |y| < 2.4, 3.5 < p_{T} < 6.5 GeV/c", "1.8<|y|<2.4, low pT", "Centrality"},
  };
  ...
  std::unique_ptr<TH1D> hPrY(loadHist(fPr.get(), "hist_eff_y", "hist_eff_y_pr"));
  std::unique_ptr<TH1D> hNpY(loadHist(fNp.get(), "hist_eff_y", "hist_eff_y_np"));
  std::unique_ptr<TH1D> hPrDenY(loadHist(fPr.get(), "hist_eff_den_y", "hist_eff_den_y_pr"));
  std::unique_ptr<TH1D> hNpDenY(loadHist(fNp.get(), "hist_eff_den_y", "hist_eff_den_y_np"));
  std::unique_ptr<TH1D> hPrNumY(loadHist(fPr.get(), "hist_eff_num_y", "hist_eff_num_y_pr"));
  std::unique_ptr<TH1D> hNpNumY(loadHist(fNp.get(), "hist_eff_num_y", "hist_eff_num_y_np"));
  if (hPrY && hNpY && hPrDenY && hNpDenY && hPrNumY && hNpNumY)
  {
    styleHist(hPrY.get(), kAzure + 2, 24, 1.7);
    styleHist(hNpY.get(), kOrange + 7, 25, 1.7);
    styleHist(hPrDenY.get(), kAzure + 2, 24, 1.7);
    styleHist(hNpDenY.get(), kOrange + 7, 25, 1.7);
    styleHist(hPrNumY.get(), kAzure + 2, 24, 1.7);
    styleHist(hNpNumY.get(), kOrange + 7, 25, 1.7);

    const TString yLabel = "cent 0-100%, 6.5 < p_{T} < 50 GeV/c";
    saveSinglePlot(hPrDenY.get(), countsYDir + "/eff_den_pr_y" + weightTag, "Prompt MC", yLabel, "|y|", "Counts", "Efficiency denominator");
    saveSinglePlot(hNpDenY.get(), countsYDir + "/eff_den_np_y" + weightTag, "Nonprompt MC", yLabel, "|y|", "Counts", "Efficiency denominator");
    saveSinglePlot(hPrNumY.get(), countsYDir + "/eff_num_pr_y" + weightTag, "Prompt MC", yLabel, "|y|", "Counts", "Efficiency numerator");
    saveSinglePlot(hNpNumY.get(), countsYDir + "/eff_num_np_y" + weightTag, "Nonprompt MC", yLabel, "|y|", "Counts", "Efficiency numerator");
    saveSinglePlot(hPrY.get(), effYDir + "/eff_pr_y" + weightTag, "Prompt MC", yLabel, "|y|", "Efficiency", "Efficiency");
    saveSinglePlot(hNpY.get(), effYDir + "/eff_np_y" + weightTag, "Nonprompt MC", yLabel, "|y|", "Efficiency", "Efficiency");
    saveOverlayPlot(hPrY.get(), hNpY.get(), overlayYDir + "/eff_pr_np_y" + weightTag, yLabel, "|y|");
    saveSummaryPlot({{hPrY.get(), hNpY.get()}}, {TString("0.4-bin |y|")}, summaryDir + "/eff_pr_np_y_summary" + weightTag, "|y|");
  }
}
