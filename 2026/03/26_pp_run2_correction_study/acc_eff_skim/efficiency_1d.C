                   ...
                   bool isPtW = true, bool useLegacy4PtWInputs = false)
{
  ...
  if (isPtW)
  {
    if (useLegacy4PtWInputs)
    {
      ...
      cout << "[INFO] pT reweight inputs (legacy 4-file mode): "
           << ptWeightMidPath << "::" << ptWeightMidName << ", "
           << ptWeightFwdPath << "::" << ptWeightFwdName << "\n";
    }
    else
  ...
  std::string outFilePath = Form(
      ...
      outDir, mcLabel.empty() ? "" : ("_" + mcLabel).c_str(), weightLabel.c_str());
  ...
  for (long iev = 0; iev < nEvt; ++iev)
  {
    ...
    for (Int_t irqq = 0; irqq < Gen_QQ_size; ++irqq)
    {
      ...
      if (absGenY >= 1.6 && absGenY < 2.4 && gen_pt >= 3.5f && gen_pt < 40.f)
      {
        if (Centrality < 20)
      ...
      if (absGenY >= 1.6 && absGenY < 2.4 && gen_pt >= 3.5f && gen_pt < 6.5f)
    ...
    if (bestRecoIdx >= 0)
    {
      ...
      if (isTnPW)
      {
        if (!ComputePbPbTnPWeight(bestRecoPt1, bestRecoEta1, Reco_mu_trig[bestRecoMuPlIdx],
      ...
      if (bestRecoAbsY >= 1.6 && bestRecoAbsY < 2.4 && bestRecoPt >= 3.5f && bestRecoPt < 40.f)
      {
        if (Centrality < 20)
        {
      ...
      if (bestRecoAbsY >= 1.6 && bestRecoAbsY < 2.4 && bestRecoPt >= 3.5f && bestRecoPt < 6.5f)
      {
  ...
  TH1D *hist_eff_y = BuildWeightedEfficiencyHist(hist_eff_num_y, hist_eff_den_y, "hist_eff_y", ";|y|;Efficiency");
