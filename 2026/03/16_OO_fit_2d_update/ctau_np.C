...
void ctau_np(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4)
{
	...
	// Build up to three positive lifetime components that share the prompt resolution kernel.
		...
		RooRealVar NsignalSS1("NsignalSS1", "NsignalSS1", 0.8 * data->numEntries(), 0.0 * data->numEntries(), std::max(1, data->numEntries()));
		RooRealVar NsignalSS2("NsignalSS2", "NsignalSS2", 0.5 * data->numEntries(), 0.0, std::max(1, data->numEntries()));
		RooRealVar NsignalSS3("NsignalSS3", "NsignalSS3", 0.20 * data->numEntries(), 0.0, std::max(1, data->numEntries()));
