...
void fit2d(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4){
	...
	const int overrideBkgErrPdfOpt = -1;
	const int overrideSigErrPdfOpt = -1;
	...
	bkg_sym_lifetime.setConstant(true);
	bkg_sym_lifetime2.setConstant(true);
	bkg_sym_lifetime3.setConstant(true);
	...
	if (nBkgFlipComponents >= 1)
		addConstraint("bkg_flip_lifetime", bkg_flip_lifetime,
			...
			constraintSigmaFloor(bkg_flip_lifetime.getVal(),
	...
	if (nBkgFlipComponents >= 2)
		addConstraint("bkg_flip_lifetime2", bkg_flip_lifetime2,
			...
			constraintSigmaFloor(bkg_flip_lifetime2.getVal(),
	...
	if (nBkgFlipComponents >= 3)
		addConstraint("bkg_flip_lifetime3", bkg_flip_lifetime3,
			...
			constraintSigmaFloor(bkg_flip_lifetime3.getVal(),
	...
	if (useErrSigLandau1) signalErrPdf->plotOn(sigTimeErrPlot, Components(sigTimeErrTail), LineColor(kBlue + 2), LineStyle(kDashed), LineWidth(2), Name("tail"));
	if (useErrSigLandau2) signalErrPdf->plotOn(sigTimeErrPlot, Components(sigTimeErrTail2), LineColor(kOrange + 7), LineStyle(kDashed), LineWidth(2), Name("tail2"));
	...
	if (useErrSigGaus1) signalErrPdf->plotOn(sigTimeErrPlot, Components(sigTimeErrGaus1), LineColor(kGreen + 2), LineStyle(kDashed), LineWidth(2), Name("gaus1"));
	if (useErrSigGaus2) signalErrPdf->plotOn(sigTimeErrPlot, Components(sigTimeErrGaus2), LineColor(kRed + 1), LineStyle(kDashed), LineWidth(2), Name("gaus2"));
