...
void mc_mass(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4, bool isBkg = false)
{
	...
	int nBkgChebyOrder = 2;
	if (yLow == 0.0f)
	{
		if (ptLow == 7.0f && ptHigh == 8.0f) //dummy
		{
			nSignalCBComponents = 2;    // 0~2
			nSignalGaussComponents = 2; // 0~2
	...
	if (isBkg && nBkgExpComponents + nBkgChebyOrder <= 0)
	{
		std::cerr << "ERROR: background is enabled but no background model is configured." << std::endl;
		return;
	}
	...
	const char *DATA_ROOT =
			"/data/users/pjgwak/work/daily_code_tracker/2026/02/00_OO_oniaskim/onia_to_skim/roodataset_roots/"
			"RooDataSet_miniAOD_isMC1_PR_Jpsi_cent0_200_Effw0_Accw0_PtW0_TnP0_OO25_HLT_OxyL1SingleMuOpen_v1.root";
	...
	TString cutAll = Form("(mass>2.6 && mass<3.5) && (pt > %g && pt < %g) && (fabs(y) > %g && fabs(y) < %g)",
	...
	{
		std::cerr << "ERROR: no entries after selection: " << cutAll << std::endl;
		return;
	}
	...
	RooFitResult *frMass = mass_pdf->fitTo(*data, Extended(), Save(), PrintLevel(0), SumW2Error(isWeight), Minimizer("Minuit2", "migrad"));
	if (!frMass || frMass->status() != 0 || frMass->covQual() < 2)
	{
		delete frMass;
		frMass = mass_pdf->fitTo(*data, Extended(), Save(), PrintLevel(0), SumW2Error(isWeight));
	}
	if (frMass)
		frMass->Print();

	{
		...
		if (!outFile.IsZombie())
		{
			...
			auto writeAbsReal = [&](const char *name, const RooAbsReal &var)
			{
				double err = 1e-6;
				if (frMass)
					err = std::max(var.getPropagatedError(*frMass), 1e-6);
				TParameter<double>(name, var.getVal()).Write();
	...
	}
	{
		...
		int hesse = -1;
		if (frMass)
			for (UInt_t i = 0, n = frMass->numStatusHistory(); i < n; ++i)
				if (auto lab = frMass->statusLabelHistory(i); lab)
				{
					if (!strcmp(lab, "MINIMIZE") || !strcmp(lab, "MIGRAD"))
						minimize = frMass->statusCodeHistory(i);
					else if (!strcmp(lab, "HESSE"))
						hesse = frMass->statusCodeHistory(i);
				}
	...
	}
	{
		...
		auto printVar = [&](const char *title, const RooAbsReal &var)
		{
			...
			else
			{
				double err = 0.0;
				if (frMass)
					err = var.getPropagatedError(*frMass);
	...
	int npar = frMass ? frMass->floatParsFinal().getSize() : 0;
