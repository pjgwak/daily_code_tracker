...
void ctau_bkg(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4)
{
	...
	if (time_result && (time_result->status() != 0 || time_result->covQual() < 2))
	{
		const double refitSigmaScale = 1.0;
		RooArgSet refitConstraints;
		RooArgList floatPars(time_result->floatParsFinal());
		auto modelVars = std::unique_ptr<RooArgSet>(time_pdf.getVariables());
		...
		for (int i = 0; i < floatPars.getSize(); ++i)
		{
			...
			refitConstraints.add(*refitPdfs.back());
		}
		if (refitConstraints.getSize() > 0)
		{
			delete time_result;
			time_result = time_pdf.fitTo(*data, Extended(), Save(), SumW2Error(isWeight), ExternalConstraints(refitConstraints));
			std::cout << "[OK] ctau_bkg constrained refit done" << std::endl;
		}
	}
	time_result->Print();
