...
void mc_mass(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4, bool isBkg = false)
{
	...
	RooRealVar Nsig("Nsig", "Nsig", std::max(1.0, 0.9 * data->numEntries()), 0.7*data->numEntries(), data->numEntries());
	RooRealVar Nbkg("Nbkg", "Nbkg", std::max(1.0, 0.1 * data->numEntries()), 0.0, data->numEntries());
