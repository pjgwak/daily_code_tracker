...
void err(float ptLow = 12, float ptHigh = 14, float yLow = 1.6, float yHigh = 2.4, bool doErrPdfFit = true)
{
  ...
  // constrained refit for errPdf fits
  const bool enableErrPdfConstrainedRefit = true;
  const double errPdfRefitSigmaScale = 0.5;
  const int errPdfConstrainedRefitMaxTries = 1;
  ...
  std::vector<std::unique_ptr<RooAbsArg>> errRetryOwned;
  auto build_err_retry_constraints = [&](RooAbsPdf &pdf, RooAbsData &data, RooFitResult *fr, RooArgSet &out, double sigmaScale)
  {
    if (!fr)
      return;
    ...
    if (!params)
      return;
    RooArgList floats(fr->floatParsFinal());
    for (int i = 0; i < floats.getSize(); ++i)
    {
      auto *src = dynamic_cast<RooRealVar *>(floats.at(i));
      if (!src)
        continue;
      auto *dst = dynamic_cast<RooRealVar *>(params->find(src->GetName()));
      if (!dst)
        continue;
      const double val = src->getVal();
      const double err = sigmaScale * src->getError();
      if (!(err > 0.0))
        continue;
      dst->setVal(val);
      dst->setConstant(false);
      auto mean = std::make_unique<RooConstVar>(Form("err_retry_%s_mean", dst->GetName()), "", val);
      auto sigma = std::make_unique<RooConstVar>(Form("err_retry_%s_sigma", dst->GetName()), "", err);
      auto constr = std::make_unique<RooGaussian>(Form("err_retry_constr_%s", dst->GetName()), "", *dst, *mean, *sigma);
      out.add(*constr);
      errRetryOwned.push_back(std::move(mean));
      errRetryOwned.push_back(std::move(sigma));
      errRetryOwned.push_back(std::move(constr));
    }
  };
  ...
  auto refitBkg = [&]() -> void
  {
    ...
    if (enableErrPdfConstrainedRefit && fit_failed(fitErrBkg.get()))
    {
      for (int itry = 0; itry < errPdfConstrainedRefitMaxTries; ++itry)
      {
        RooArgSet retryConstraints;
        build_err_retry_constraints(*pdfErrCombo, *hErrBkg, fitErrBkg.get(), retryConstraints, errPdfRefitSigmaScale);
        if (retryConstraints.getSize() <= 0)
          break;
        fitErrBkg = run_bkg_fit(&retryConstraints);
        if (fitErrBkg)
        {
          printf("[OK] err bkg constrained refit attempt %d done\n", itry + 1);
          fitErrBkg->Print("V");
          fitErrBkgPtr = fitErrBkg.get();
        }
        if (!fit_failed(fitErrBkg.get()))
          break;
      }
    }
  };
  ...
  if (doErrPdfFit)
  {
    ...
    if (enableErrPdfConstrainedRefit && fit_failed(fitErrSig.get()))
    {
      for (int itry = 0; itry < errPdfConstrainedRefitMaxTries; ++itry)
      {
        RooArgSet retryConstraints;
        build_err_retry_constraints(*pdfErrComboSig, *hErrSig, fitErrSig.get(), retryConstraints, errPdfRefitSigmaScale);
        if (retryConstraints.getSize() <= 0)
          break;
        fitErrSig = run_sig_fit(&retryConstraints);
        if (fitErrSig)
        {
          printf("[OK] err sig constrained refit attempt %d done\n", itry + 1);
          fitErrSig->Print("V");
          fitErrSigPtr = fitErrSig.get();
        }
        if (!fit_failed(fitErrSig.get()))
          break;
      }
    }
  }
