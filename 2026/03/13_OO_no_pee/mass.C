...
void mass(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4)
{
	...
	if (!ctau3DTmp) {
		std::cerr << "ERROR: required variable ctau3D is missing in dataset." << std::endl;
		return;
	}
	...
	finalCut = Form(
		"%s && (ctau3D >= %g && ctau3D <= %g)",
		cutBasic.Data(), ctau3DRange.first, ctau3DRange.second
	);
	...
	RooRealVar signal_mass_frac_ratio2("signal_mass_frac_ratio2", "signal_mass_frac_ratio2", 2, 1e-3, 1e3);
	RooFormulaVar signal_mass_frac2("signal_mass_frac2", "@0/(1.0+@0)", RooArgList(signal_mass_frac_ratio2));
	RooRealVar signal_mass_frac_ratio3("signal_mass_frac_ratio3", "signal_mass_frac_ratio3", 1, 1e-3, 1e3);
	...
	signal_mass_sigma2_log_offset.setVal(std::log(std::max(readDoubleParam("signal_mass_sigma2", signal_mass_sigma2.getVal()) - sigmaFloor, 1e-4)));
	...
	signal_mass_cb_sigma_log_offset->setVal(std::log(std::max(readDoubleParam("signal_mass_cb_sigma", signal_mass_cb_sigma->getVal()) - sigmaFloor, 1e-4)));
	signal_mass_cb_sigma2_log_offset.setVal(std::log(std::max(readDoubleParam("signal_mass_cb_sigma2", signal_mass_cb_sigma2.getVal()) - sigmaFloor, 1e-4)));
	...
	signal_mass_cb_n2.setVal(readDoubleParam("signal_mass_cb_n2", signal_mass_cb_n2.getVal()));
	signal_mass_frac_ratio1.setVal(readDoubleParam("signal_mass_frac_ratio1", signal_mass_frac_ratio1.getVal()));
	signal_mass_frac_ratio2.setVal(readDoubleParam("signal_mass_frac_ratio2", signal_mass_frac_ratio2.getVal()));
	signal_mass_frac_ratio3.setVal(readDoubleParam("signal_mass_frac_ratio3", signal_mass_frac_ratio3.getVal()));
	...
	RooRealVar Nsig("Nsig", "Nsig", std::max(1.0, 0.5 * data->numEntries()), 0.0, data->numEntries());
	RooRealVar Nbkg("Nbkg", "Nbkg", std::max(1.0, 0.5 * data->numEntries()), 0.0, data->numEntries());
	// Reparameterize Cheby coefficients as p_i = tanh(u_i) so the effective
	// coefficients stay in (-1, 1) while the minimizer moves in a smoother raw space.
	const double chebyRawLimit = 4.0;
	RooRealVar bkg_mass_p1_raw("bkg_mass_p1_raw", "bkg_mass_p1_raw", 0.01, -chebyRawLimit, chebyRawLimit);
	RooRealVar bkg_mass_p2_raw("bkg_mass_p2_raw", "bkg_mass_p2_raw", 0.01, -chebyRawLimit, chebyRawLimit);
	RooRealVar bkg_mass_p3_raw("bkg_mass_p3_raw", "bkg_mass_p3_raw", 0.01, -chebyRawLimit, chebyRawLimit);
	RooRealVar bkg_mass_p4_raw("bkg_mass_p4_raw", "bkg_mass_p4_raw", 0.01, -chebyRawLimit, chebyRawLimit);
	RooRealVar bkg_mass_p5_raw("bkg_mass_p5_raw", "bkg_mass_p5_raw", 0.01, -chebyRawLimit, chebyRawLimit);
	RooRealVar bkg_mass_p6_raw("bkg_mass_p6_raw", "bkg_mass_p6_raw", 0.01, -chebyRawLimit, chebyRawLimit);
	...
	auto addConstraint = [&](const char *name, RooRealVar &var, bool freeThis)
	{
		if (freeThis)
			return;
		const TString baseName(name);
		const double central = readDoubleParam(baseName.Data(), var.getVal());
		const double error = readDoubleParam((baseName + "_err").Data(), 0.0);
		if (!std::isfinite(error) || error <= 0.0)
			return;
		var.setVal(central);
		const TString meanName = TString::Format("ext_%s_mc_mean", baseName.Data());
		const TString sigmaName = TString::Format("ext_%s_mc_sigma", baseName.Data());
		const TString pdfName = TString::Format("%s_mc_constraint", baseName.Data());
	...
	auto addConstraintAbsReal = [&](const char *name, RooAbsReal &var, bool freeThis)
	{
		if (freeThis)
			return;
		const TString baseName(name);
	...
	if (nSignalGaussComponents >= 1)
		addConstraint("signal_mass_sigma", signal_mass_sigma, true);
	if (nSignalCBComponents >= 1)
	{
			if (nSignalGaussComponents >= 1)
				addConstraintAbsReal("signal_mass_cb_sigma_delta", *signal_mass_cb_sigma_delta, false);
			else
				addConstraint("signal_mass_cb_sigma", signal_mass_cb_sigma_base, true);
	}
	if (nSignalGaussComponents >= 2)
		addConstraintAbsReal("signal_mass_sigma_delta2", signal_mass_sigma_delta2, false);
	if (nSignalCBComponents >= 2)
		addConstraintAbsReal("signal_mass_cb_sigma_delta2", signal_mass_cb_sigma_delta2, false);
	if (nSignalCBComponents >= 1)
	{
		addConstraint("signal_mass_cb_alpha", signal_mass_cb_alpha, false);
		addConstraint("signal_mass_cb_n", signal_mass_cb_n, false);
	}
	if (nSignalCBComponents >= 2)
	{
		addConstraint("signal_mass_cb_alpha2", signal_mass_cb_alpha2, false);
		addConstraint("signal_mass_cb_n2", signal_mass_cb_n2, false);
	}
	if (signalComponents.size() >= 2)
		addConstraint("signal_mass_frac_ratio1", signal_mass_frac_ratio1, false);
	if (signalComponents.size() >= 3)
		addConstraint("signal_mass_frac_ratio2", signal_mass_frac_ratio2, false);
	if (signalComponents.size() >= 4)
		addConstraint("signal_mass_frac_ratio3", signal_mass_frac_ratio3, false);
...

	{
		...
		if (!outFile.IsZombie())
		{
			...
			if (nBkgChebyOrder >= 1)
			{
				writeVar("bkg_mass_p1_raw", bkg_mass_p1_raw);
				writeAbsReal("bkg_mass_p1", bkg_mass_p1);
			}
			if (nBkgChebyOrder >= 2)
			{
				writeVar("bkg_mass_p2_raw", bkg_mass_p2_raw);
				writeAbsReal("bkg_mass_p2", bkg_mass_p2);
			}
			if (nBkgChebyOrder >= 3)
			{
				writeVar("bkg_mass_p3_raw", bkg_mass_p3_raw);
				writeAbsReal("bkg_mass_p3", bkg_mass_p3);
			}
			if (nBkgChebyOrder >= 4)
			{
				writeVar("bkg_mass_p4_raw", bkg_mass_p4_raw);
				writeAbsReal("bkg_mass_p4", bkg_mass_p4);
			}
			if (nBkgChebyOrder >= 5)
			{
				writeVar("bkg_mass_p5_raw", bkg_mass_p5_raw);
				writeAbsReal("bkg_mass_p5", bkg_mass_p5);
			}
			if (nBkgChebyOrder >= 6)
			{
				writeVar("bkg_mass_p6_raw", bkg_mass_p6_raw);
				writeAbsReal("bkg_mass_p6", bkg_mass_p6);
			}
			if (mass_result)
			...
			for (UInt_t i = 0, n = mass_result->numStatusHistory(); i < n; ++i)
			{
				...
				if (lab)
				{
					if (!strcmp(lab, "MINIMIZE") || !strcmp(lab, "MIGRAD"))
						minimize = mass_result->statusCodeHistory(i);
	...
	}
	{
		...
		auto printVar = [&](const char *title, const RooAbsReal &var)
		{
			...
			else
			{
				double err = 0.0;
				if (mass_result)
					err = var.getPropagatedError(*mass_result);
		...
		printVar("mean", signal_mass_mean);
			if (nSignalGaussComponents >= 1)
