...
void err2(float ptLow = 14, float ptHigh = 20, float yLow = 1.6, float yHigh = 2.4)
{
  ...
  RooRealVar sigErrGaus1Mean("sigErrGaus1Mean", "sigErrGaus1Mean", gaus1MeanInit, errRange.first, errRange.second);
  RooRealVar sigErrGaus1Sigma("sigErrGaus1Sigma", "sigErrGaus1Sigma", gaus1SigmaInit, 5e-4, std::max(0.15 * errSpan, 2e-3));
  RooRealVar sigErrGaus2Mean("sigErrGaus2Mean", "sigErrGaus2Mean", gaus2MeanInit, errRange.first, errRange.second);
  RooRealVar sigErrGaus2Sigma("sigErrGaus2Sigma", "sigErrGaus2Sigma", gaus2SigmaInit, 1e-3, std::max(0.30 * errSpan, 4e-3));
  RooRealVar sigErrTailMpv("sigErrTailMpv", "sigErrTailMpv", tailMpvInit, errRange.first, errRange.second);
  RooRealVar sigErrTailWidth("sigErrTailWidth", "sigErrTailWidth", tailWidthInit, 1e-3, std::max(0.50 * errSpan, 5e-3));
  RooRealVar sigErrTail2Mpv("sigErrTail2Mpv", "sigErrTail2Mpv", tail2MpvInit, errRange.first, errRange.second);
  RooRealVar sigErrTail2Width("sigErrTail2Width", "sigErrTail2Width", tail2WidthInit, 1e-3, std::max(0.80 * errSpan, 8e-3));
  RooRealVar sigErrLognM0("sigErrLognM0", "sigErrLognM0", lognM0Init, std::max(errRange.first, 1e-6), errRange.second);
  RooRealVar sigErrLognK("sigErrLognK", "sigErrLognK", lognKInit, 0.05, 0.95);
  RooRealVar sigErrCore1Frac("sigErrCore1Frac", "sigErrCore1Frac", 0.60, 0.05, 0.95);
  RooRealVar sigErrTailFrac("sigErrTailFrac", "sigErrTailFrac", 0.08, 0.001, 0.30);
  RooRealVar sigErrTail2Frac("sigErrTail2Frac", "sigErrTail2Frac", 0.05, 0.001, 0.30);
  RooRealVar sigErrLognFrac("sigErrLognFrac", "sigErrLognFrac", 0.03, 0.0005, 0.20);
  ...
  RooAddPdf sigTimeErrPdf(
      "sigTimeErrPdf", "sigTimeErrPdf",
      RooArgList(sigErrTail, sigErrTail2, sigErrLogn, sigErrGaus1, sigErrGaus2),
      RooArgList(sigErrTailFrac, sigErrTail2Frac, sigErrLognFrac, sigErrCore1Frac),
      true);
  ...
  }
  {
    ...
    tx.SetTextSize(0.03);
    tx.SetTextFont(42);
    const int status = sigTimeErrResult ? sigTimeErrResult->status() : -1;
    int hesse = -1;
    if (sigTimeErrResult)
    {
  ...
  TParameter<int>("nTimeErrComponents", 5).Write();
