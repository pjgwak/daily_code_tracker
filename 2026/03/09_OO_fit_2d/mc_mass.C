...
void mc_mass(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4, bool isBkg = true)
{
	bool isWeight = false;
	// ------------------------------------------------------------------
	// model control
	...
	std::unique_ptr<RooFitResult> mass_result(mass_pdf->fitTo(*data, Extended(), Save(), PrintLevel(-1), SumW2Error(isWeight)));
	mass_result->Print();
