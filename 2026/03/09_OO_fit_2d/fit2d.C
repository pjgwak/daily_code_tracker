...
void fit2d(float ptLow = 1, float ptHigh = 2, float yLow = 1.6, float yHigh = 2.4){
	...
	RooRealVar signal_mass_frac1("signal_mass_frac1","signal_mass_frac1",readDoubleParam(*massFile,"signal_mass_frac1",0.65));
	RooRealVar signal_mass_frac2("signal_mass_frac2","signal_mass_frac2",readDoubleParam(*massFile,"signal_mass_frac2",0.20));
	RooRealVar signal_mass_frac3("signal_mass_frac3","signal_mass_frac3",readDoubleParam(*massFile,"signal_mass_frac3",0.10));
	signal_mass_frac1.setConstant(true);
	signal_mass_frac2.setConstant(true);
	signal_mass_frac3.setConstant(true);
	...
	if (nSignalCBComponents >= 2) signalMassPdfList.add(signal_mass_cb2);
	...
	if (signalMassPdfList.getSize() == 1) signal_mass_pdf = static_cast<RooAbsPdf *>(&signal_mass_gaus);
	if (signalMassPdfList.getSize() == 1 && nSignalGaussComponents == 0) signal_mass_pdf = static_cast<RooAbsPdf *>(&signal_mass_cb);
	if (!signal_mass_pdf) {
	...
	RooRealVar signal_log_lifetime_offset(
		...
		std::log(1e-2),
		std::log(std::max(signalLifetimeCeil - signalLifetimeFloorSaved, 2e-2)));
	...
	RooRealVar signal2_log_lifetime_offset(
		...
		std::log(1e-2),
		std::log(std::max(signalLifetime2Ceil - signalLifetime2Floor, 2e-2)));
	...
	RooRealVar signal3_log_lifetime_offset(
		...
		std::log(1e-2),
		std::log(std::max(signalLifetime3Ceil - signalLifetime3Floor, 2e-2)));
