...
void mc_mass(float ptLow = 12, float ptHigh = 20,
          float yLow = 0.0, float yHigh = 2.4,
          int bins = 200,
          int nSignalCBComponentsArg = 2,
          int nSignalGaussComponentsArg = 3,
          ...
          int maxEntries = -1)
{
  ...
  if (maxEntries > 0 && data->numEntries() > maxEntries) {
    data.reset(static_cast<RooDataSet *>(data->reduce(EventRange(0, maxEntries))));
  }
  ...
  const double meanSeed = 3.09715;
  const double sigmaG1Seed = 0.0193;
  const double sigmaG2Seed = 0.0368;
  const double sigmaG3Seed = 0.0620;
  const double sigmaCB1Seed = 0.0633;
  const double alphaCB1Seed = 0.815;
  const double nCB1Seed = 2.11;
  const double sigmaCB2Seed = 0.0375;
  const double alphaCB2Seed = -1.70;
  const double nCB2Seed = 1.50;
  const double fracG1Seed = 0.194;
  const double fracCB1Seed = 0.074;
  const double fracCB2Seed = 0.080;
  const double fracG2Seed = 0.761;

  RooRealVar mean("mean", "Jpsi mean", meanSeed, 3.05, 3.15);
  RooRealVar sigmaG1("sigmaG1", "G1 sigma", sigmaG1Seed, 0.001, 0.080);
  RooRealVar sigmaG2("sigmaG2", "G2 sigma", sigmaG2Seed, 0.005, 0.200);
  RooRealVar sigmaG3("sigmaG3", "G3 sigma", sigmaG3Seed, 0.020, 0.900);
  RooRealVar sigmaCB1("sigmaCB1", "CB left sigma", sigmaCB1Seed, 0.003, 0.120);
  RooRealVar sigmaCB2("sigmaCB2", "CB right sigma", sigmaCB2Seed, 0.003, 0.120);
  ...
  RooRealVar alphaCB1("alphaCB1", "CB left alpha", alphaCB1Seed, 0.20, 6.0);
  RooRealVar nCB1("nCB1", "CB left n", nCB1Seed, 1.01, 60.0);
  RooCBShape sigCB1("sigCB1", "signal CB left", mass, mean, sigmaCB1, alphaCB1, nCB1);

  RooRealVar alphaCB2("alphaCB2", "CB right alpha", alphaCB2Seed, -6.0, -0.20);
  RooRealVar nCB2("nCB2", "CB right n", nCB2Seed, 1.01, 60.0);
  ...
  RooRealVar fracG2("fracG2", "G2 fraction", fracG2Seed, 1e-4, 1.0 - 1e-4);
  ...
  if (sigList.getSize() >= 5) sigFracList.add(fracG2);
  ...
  auto result = std::unique_ptr<RooFitResult>(
      sigMass.fitTo(*data, Extended(false), Save(true), PrintLevel(-1),
                    Strategy(1), Offset(true), NumCPU(12),
  ...
  if (!result || result->status() != 0 || result->covQual() < 2) {
    result.reset(sigMass.fitTo(*data, Extended(false), Save(true), PrintLevel(-1),
                               Strategy(2), Offset(true), NumCPU(12),
                               RecoverFromUndefinedRegions(1.0)));
  }
  if (!result) {
  ...
  TParameter<int>("maxEntries", maxEntries).Write();
