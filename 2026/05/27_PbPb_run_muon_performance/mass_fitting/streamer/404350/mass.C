...
void mass(float ptLow = 6.5, float ptHigh = 50,
                             ...
                             int bkgOrder = 1,
                             int bins = 90,
                             int nSignalCBComponentsArg = 1,
                             int nSignalGaussComponentsArg = 0,
                             int nBkgExpComponentsArg = 0,
                             int nBkgChebyOrderArg = 2,
                             bool fixSignalShapeFromMc = false)
{
  // ------------------------------------------------------------------
  // run and trigger configuration
  // ------------------------------------------------------------------
  const char *pdName = "Streamer";
  const int runNumber = 404350;
  const char *triggerName = "HLT_HIMinimumBiasHF1AND_v";
  ...
  const char *dataRoot =
      ...
      "skimming/roodataset_roots/Streamer/"
      "RooDataSet_miniAOD_isMC0_globalOff_Jpsi_cent0_200_Effw0_Accw0_PtW0_TnP0_Streamer_run404350_MinBias.root";
      // "RooDataSet_miniAOD_isMC0_globalOff_Jpsi_cent0_200_Effw0_Accw0_PtW0_TnP0_Streamer_run404338_SingleMu0_Open.root";
  ...
  const auto sigmaG1Range = rangeAround(sigmaG1Seed, 0.001, 0.300, 0.75, 3.0, 0.008, 0.030);
  ...
  const auto sigmaG2Range = rangeAround(sigmaG2Seed, 0.001, 0.500, 0.75, 3.0, 0.015, 0.060);
  ...
  const double nCB1Seed = readMcMass("nCB1", 3.0);
  const auto nCB1Range = rangeAround(nCB1Seed, 0.01, 500.0, 0.75, 5.0, 0.50, 10.0);
  ...
  legend.Draw("same");
  {
    ...
    tx.SetTextFont(42);
    tx.SetTextAlign(31);
    tx.DrawLatex(0.96, 0.935, Form("PbPb %s run %d", pdName, runNumber));
  }
  {
    TLatex tx;
    tx.SetNDC();
