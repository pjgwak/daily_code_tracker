...
void mass_ttree_fallback(float ptLow = 1, float ptHigh = 50,
                             ...
                             bool fixSignalShapeFromMc = false)
{
  ...
  if (inputData) {
    data.reset(static_cast<RooDataSet *>(inputData->reduce(cut)));
  } else {
    printf("Cannot read RooDataSet named dataset, trying TTree fallback\n");
    auto tree = dynamic_cast<TTree *>(inputFile->Get("RooTreeDataStore_dataset_"));
    if (!tree) {
      printf("Cannot find fallback TTree RooTreeDataStore_dataset_\n");
      return;
    }
    ...
    double treePt = 0.0;
    double treeY = 0.0;
    tree->SetBranchAddress("mass", &treeMass);
    tree->SetBranchAddress("pt", &treePt);
    tree->SetBranchAddress("y", &treeY);

    RooRealVar massIn("mass", "mass", 2.5, 4.5, "GeV/c^{2}");
    RooRealVar ptIn("pt", "pt", 0.0, 300.0, "GeV/c");
    RooRealVar yIn("y", "rapidity", -3.0, 3.0);
    RooArgSet argSet(massIn, ptIn, yIn);
    data = std::make_unique<RooDataSet>("dataset_cut", "dataset_cut", argSet);

    const Long64_t nEntries = tree->GetEntries();
    for (Long64_t i = 0; i < nEntries; ++i) {
      tree->GetEntry(i);
      if (!(treeMass > 2.6 && treeMass < 3.5 &&
            treePt > ptLow && treePt < ptHigh &&
            std::abs(treeY) > yLow && std::abs(treeY) < yHigh)) {
        continue;
      }
      massIn.setVal(treeMass);
      ptIn.setVal(treePt);
      yIn.setVal(treeY);
      data->add(argSet);
    }
    printf("TTree fallback selected entries: %lld / %lld\n",
           (Long64_t)data->numEntries(), nEntries);
  }
