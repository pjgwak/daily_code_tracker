...
void skim_to_roodataset(
    ...
    std::string recoLabel = "PromptReco", std::string userLabel = "404337_404474_SingleMu0_Open",
    // std::string recoLabel = "McPrompt2023", std::string userLabel = "HLT_HIL1SingleMu0_Open",
    // HLT_HIL1SingleMu0_Open_v
    bool fAccW = false, bool fEffW = false,
    bool isTnP = false, bool isPtW = false,
    int cLow = 0, int cHigh = 200,
    float massLow = 2.6, float massHigh = 4.5,
    ...
    int hiHFBinEdge = 0)
{
  ...
  const std::string skimInputDir = recoLabel.empty() ? skimRootDir : skimRootDir + "/" + recoLabel;
  const std::string userLabelPart = userLabel.empty() ? "" : "_" + userLabel;
  const std::string sampleLabelPart = recoLabel.empty() ? userLabelPart : "_" + recoLabel + userLabelPart;
  ...
  const std::string mcLabelPart = isMC && !MCtype_tag.IsNull() ? "_" + std::string(MCtype_tag.Data()) : "";
  std::string inputFile = Form("%s/skim_PbPb2026_isMC%d%s_%s_Dimuon_MiniAOD_%s%s.root",
                               skimInputDir.c_str(), isMC, mcLabelPart.c_str(),
                               globalTag.Data(), recoLabel.c_str(), userLabelPart.c_str());
  ...
  float zVtx = 0.f, mass[nMaxDimu] = {0.f}, pt[nMaxDimu] = {0.f}, pt1[nMaxDimu] = {0.f}, pt2[nMaxDimu] = {0.f}, y[nMaxDimu] = {0.f};
  ...
  if (hasBranch("pt1")) muon_chain->SetBranchAddress("pt1", pt1);
  if (hasBranch("pt2")) muon_chain->SetBranchAddress("pt2", pt2);
  ...
  // output file
  // ------------------------------------------------------------------
  const std::string rooRootDir = "roodataset_roots";
  const std::string rooOutputDir = recoLabel.empty() ? rooRootDir : rooRootDir + "/" + recoLabel;
  gSystem->mkdir(rooOutputDir.c_str(), true);
  TFile *out_file = nullptr;
  if (isMC)
  {
    out_file = new TFile(Form("%s/RooDataSet_miniAOD_isMC%d_%s_%s_Jpsi_cent%i_%i_Effw%d_Accw%d_PtW%d_TnP%d%s.root",
                              rooOutputDir.c_str(), isMC, MCtype_tag.Data(), globalTag.Data(), cLow, cHigh, fEffW, fAccW, isPtW, isTnP, sampleLabelPart.c_str()),
                         "RECREATE");
  }
  else
  {
    out_file = new TFile(Form("%s/TestRooDataSet_miniAOD_isMC%d_%s_Jpsi_cent%i_%i_Effw%d_Accw%d_PtW%d_TnP%d%s.root",
                              rooOutputDir.c_str(), isMC, globalTag.Data(), cLow, cHigh, fEffW, fAccW, isPtW, isTnP, sampleLabelPart.c_str()),
                         "RECREATE");
  }
  if (!out_file || out_file->IsZombie())
  {
    std::cout << "[ERROR] failed to create output file in RooDataSet output directory\n";
    return;
  }
  ...
  // ------------------------------------------------------------------
  TString metaInfo;
  metaInfo += Form("reco_label=%s;", recoLabel.c_str());
  metaInfo += Form("user_label=%s;", userLabel.c_str());
  metaInfo += Form("sample_label=%s;", sampleLabelPart.c_str());
