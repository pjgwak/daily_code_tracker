...
void onia_to_skim_vector(long nEvt = -1, bool isMC = false, bool isPr = false, bool useGlobal = false, std::string recoLabel = "PromptReco", std::string userLabel = "404359_404400_SingleMu0_Open")
{
  ...
  std::string inputFile = "/data/users/pjgwak/work/daily_code_tracker/2026/05/27_PbPb_run_muon_performance/oniatree/PromptReco/Oniatree_RawPrimeAll_run404359_404400.root";
  if (isMC)
  {
    inputFile = isPr
  ...
  // 21: MB, 8: single muon
  const int trigSelection = 8;
  if (trigSelection < 0 || trigSelection > 22)
  {
  ...
  for (long iev = 0; iev < nEvt; ++iev)
  {
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
    {
      ...
      if (trigSelection>=0 && trigSelection<=7)
      { 
      ...
      } else if (trigSelection>=8 && trigSelection<=20)
      {
      ...
      } else if (trigSelection>=21 && trigSelection<=22)
      {
        // skip it!
