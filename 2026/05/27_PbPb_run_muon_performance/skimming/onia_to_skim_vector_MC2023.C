// ===== Trigger info - PbPb2023 MC Prompt =====
// triggerList    = {
...
void onia_to_skim_vector_MC2023(long nEvt = -1, bool isMC = true, bool isPr = true, bool useGlobal = false, std::string recoLabel = "McPrompt2023", std::string userLabel = "HLT_HIMinimumBiasHF1AND_v")
{
  ...
  std::string inputFile = "/data/users/pjgwak/work/daily_code_tracker/2026/05/27_PbPb_run_muon_performance/oniatree/Streamer/Oniatree_PD20_29_run404359_miniAOD_streamer.root";
  if (isMC)
  {
    inputFile = isPr
  ...
  const bool hasVector4mom =
      oniaTree->GetBranch("Reco_QQ_4mom_m") &&
      oniaTree->GetBranch("Reco_QQ_4mom_pt") &&
      oniaTree->GetBranch("Reco_QQ_4mom_phi") &&
      oniaTree->GetBranch("Reco_QQ_4mom_eta") &&
      oniaTree->GetBranch("Reco_mu_4mom_pt") &&
      oniaTree->GetBranch("Reco_mu_4mom_phi") &&
      oniaTree->GetBranch("Reco_mu_4mom_eta");
  const bool hasTClones4mom =
      oniaTree->GetBranch("Reco_QQ_4mom") &&
      oniaTree->GetBranch("Reco_mu_4mom");

  oniaTree->SetBranchStatus("*", 0);
  auto enableBranch = [&](const char *name) {
    if (oniaTree->GetBranch(name))
      oniaTree->SetBranchStatus(name, 1);
  };

  enableBranch("runNb");
  enableBranch("eventNb");
  enableBranch("LS");
  enableBranch("zVtx");
  enableBranch("HLTriggers");
  enableBranch("Reco_QQ_size");
  enableBranch("Reco_QQ_trig");
  enableBranch("Reco_QQ_VtxProb");
  enableBranch("Reco_QQ_mupl_idx");
  enableBranch("Reco_QQ_mumi_idx");
  enableBranch("Reco_QQ_sign");
  enableBranch("Reco_QQ_ctau3D");
  enableBranch("Reco_QQ_ctauErr3D");
  enableBranch("Reco_QQ_ctau");
  enableBranch("Reco_QQ_ctauErr");
  enableBranch("Reco_QQ_dca");
  enableBranch("Reco_mu_trig");
  enableBranch("Reco_mu_highPurity");
  enableBranch("Reco_mu_nTrkHits");
  enableBranch("Reco_mu_nMuValHits");
  enableBranch("Reco_mu_nTrkWMea");
  enableBranch("Reco_mu_nPixWMea");
  enableBranch("Reco_mu_nPixValHits");
  enableBranch("Reco_mu_isSoftCutBased");
  enableBranch("Reco_mu_isTracker");
  enableBranch("Reco_mu_isGlobal");
  if (isMC)
  {
    enableBranch("Gen_weight");
    enableBranch("Reco_mu_whichGen");
  }

  if (hasVector4mom)
  {
    enableBranch("Reco_QQ_4mom_m");
    enableBranch("Reco_QQ_4mom_pt");
    enableBranch("Reco_QQ_4mom_phi");
    enableBranch("Reco_QQ_4mom_eta");
    enableBranch("Reco_mu_4mom_pt");
    enableBranch("Reco_mu_4mom_phi");
    enableBranch("Reco_mu_4mom_eta");
  }
  else if (hasTClones4mom)
  {
    oniaTree->SetBranchStatus("Reco_QQ_4mom*", 1);
    oniaTree->SetBranchStatus("Reco_mu_4mom*", 1);
  }
  ...
  {
    std::cerr << "[ERROR] No supported 4-vector branches found in input tree\n";
    return;
  }
  ...
  const std::string skimRootDir = "/data/users/pjgwak/work/daily_code_tracker/2026/05/27_PbPb_run_muon_performance/skimming/skim_roots";
  const std::string outputDir = recoLabel.empty() ? skimRootDir : skimRootDir + "/" + recoLabel;
  const std::string userLabelPart = userLabel.empty() ? "" : "_" + userLabel;
  gSystem->mkdir(outputDir.c_str(), true);

  TFile *fFlowSkim = new TFile(
      Form("%s/skim_PbPb2026_isMC%d%s_%s_Dimuon_MiniAOD_%s%s.root",
           outputDir.c_str(), isMC, mcLabel.empty() ? "" : ("_" + mcLabel).c_str(),
           globalLabel.c_str(), recoLabel.c_str(), userLabelPart.c_str()),
      "recreate");

  cout << "useGlobal: " << useGlobal << "\n";
  cout << "Output directory: " << outputDir << "\n";
  cout << "Reco label: " << recoLabel << "\n";
  cout << "Output file label: " << globalLabel << userLabelPart << "\n";
  ...
  // 21: MB, 8: single muon
  const int trigSelection = 24;
  // 15:single muon, 24:MB
  if (trigSelection < 0 || trigSelection > 26)
  {
    std::cerr << "[ERROR] Invalid trigSelection bit: " << trigSelection << "\n";
    return;
  }
  const ULong64_t trigMask = (1ULL << trigSelection);
  cout << "trigSelection: " << trigSelection
       << "  trigMask: " << trigMask << "\n";
  // note: if you want to minBias trigger, only apply HLT level cut.
  // Comment out both dimuon(Reco_QQ_trig) and singlel muon (Reco_mu_trig) level trigMask cut.
  ...
  for (long iev = 0; iev < nEvt; ++iev)
  {
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
    {
      ...
      else
      {
        ...
        if (irqq >= Reco_QQ_4mom->GetEntriesFast() ||
            iMuPl >= Reco_mu_4mom->GetEntriesFast() ||
            iMuMi >= Reco_mu_4mom->GetEntriesFast()) continue;

        const auto *qq4mom = static_cast<TLorentzVector *>(Reco_QQ_4mom->At(irqq));
        const auto *muPl4mom = static_cast<TLorentzVector *>(Reco_mu_4mom->At(iMuPl));
        const auto *muMi4mom = static_cast<TLorentzVector *>(Reco_mu_4mom->At(iMuMi));
        if (!qq4mom || !muPl4mom || !muMi4mom) continue;
      ...
      if (trigSelection>=0 && trigSelection<=14)
      { 
        // trigger cut for dimuon candidate - turn on if you need it.
        if ((Reco_QQ_trig[irqq] & trigMask) == 0) continue;

        // tigher condition: pass if both muons matched the trigger.
        // if ((Reco_mu_trig[iMuPl] & trigMask) == 0) continue;
        // if ((Reco_mu_trig[iMuMi] & trigMask) == 0) continue;
      } else if (trigSelection>=15 && trigSelection<=23)
      {
        // loose condition for single muon trigger: pass if either muon matched the trigger.
        if (((Reco_mu_trig[iMuPl] & trigMask) == 0) &&
            ((Reco_mu_trig[iMuMi] & trigMask) == 0)) continue;

      } else if (trigSelection>=24 && trigSelection<=26)
      {
        // skip it!
        // minbias doesn't need it.
      }
      ++recoPassMuonTrig;
