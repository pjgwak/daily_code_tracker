...
void plot_muon_distribution(bool isMC = false, bool isPr = false, bool useGlobal = false, const char *userLabel = "Streamer", const char *inputFileOverride = "")
{
  ...
  else if (std::string(userLabel) == "Streamer")
  {
    inputFile =
        Form("skim_roots/Streamer/skim_PbPb2026_isMC%d%s_%s_Dimuon_MiniAOD_Streamer.root",
             isMC, mcLabel.empty() ? "" : ("_" + mcLabel).c_str(), globalLabel.c_str());
  }
  else
  {
    inputFile =
        Form("skim_roots/skim_PbPb2026_isMC%d%s_%s_Dimuon_MiniAOD_PromptReco_%s.root",
             isMC, mcLabel.empty() ? "" : ("_" + mcLabel).c_str(), globalLabel.c_str(), userLabel);
  }
  ...
  std::cout << "[INFO] tree entries: " << myTree->GetEntries() << "\n";
  ...
  std::cout << "[INFO] histogram entries: " << h->GetEntries()
            << "  integral: " << h->Integral() << "\n";
  gPad->SetLogz();
  ...
  }
  {
    ...
    tx.SetTextFont(42);
    tx.SetTextAlign(33);
    tx.DrawLatex(0.9, 0.985, "PbPb 2026");
  }
  {
    TLatex tx;
    tx.SetNDC();
    ...
    tx.DrawLatex(0.18, 0.875, userLabel);
  }
  ...
  const std::string outputPdfName =
      Form("figs/soft_muon_pT_eta_both_isMC%d%s_%s_%s.pdf",
           isMC, mcLabel.empty() ? "" : ("_" + mcLabel).c_str(), globalLabel.c_str(), userLabel);
  std::cout << "[INFO] output file: " << outputPdfName << "\n";
  c1.Update();
