...
void onia_to_skim_vector_yellow(long nEvt = -1, bool isMC = false, bool isPr = false, bool useGlobal = true, std::string recoLabel = "PromptReco", std::string userLabel = "404337_404474_SingleMu0_Open_yellow", std::string inputSubdir = "404337_404474_yellow")
{
  // run404350_SingleMu0_Open, run404350_MinBias
  ...
  // std::string inputFile = "/data/users/pjgwak/work/daily_code_tracker/2026/05/27_PbPb_run_muon_performance/oniatree/Streamer/Oniatree_PDAll_run404430_miniAOD_streamer.root";
  std::string inputFile = "/data/users/pjgwak/work/daily_code_tracker/2026/05/27_PbPb_run_muon_performance/oniatree/" + recoLabel + "/" + inputSubdir + "/*.root";
  if (isMC)
  {
    inputFile = isPr
  ...
  const int nInputFiles = oniaTree->Add(inputFile.c_str());
  cout << "Input file pattern: " << inputFile << "\n";
  cout << "Added input files: " << nInputFiles << "\n";
  if (nInputFiles == 0)
  {
    std::cerr << "[ERROR] No input ROOT files found: " << inputFile << "\n";
    return;
  }
  ...
  // 21: MB, 8: single muon
  const int trigSelection = 0;
  if (trigSelection < 0 || trigSelection > 16)
  {
  ...
  for (long iev = 0; iev < nEvt; ++iev)
  {
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
    {
      ...
      if (!(pt1_ > 4 && pt2_ > 4 && abs(eta1_) < 2.4 && abs(eta2_) < 2.4))
        continue;
      ...
      // if (trigSelection>=0 && trigSelection<=3)
      // { 
      ...
      // } else if (trigSelection>=4 && trigSelection<=14)
      // {
      ...
      // } else if (trigSelection>=15 && trigSelection<=16)
      // {
      //   // skip it!
